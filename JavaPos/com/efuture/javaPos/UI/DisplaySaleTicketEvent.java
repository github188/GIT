...
public class DisplaySaleTicketEvent 
{
	...
	public DisplaySaleTicketEvent(DisplaySaleTicketForm dstf,String date,String code,boolean flaglist,int type)
	{
		...
		String help_message = Language.apply("'付款键'切换商品/付款");
		dstbs = CustomLocalize.getDefault().createDisplaySaleTicketBS();
		
		switch (this.type)
		{
			case StatusType.MN_XSCX:
				help_message += Language.apply(",'打印键'重打小票");
				break;
			case StatusType.MN_MODIFYINVNO:
				help_message += Language.apply(",'打印键'重打小票");
				help_message += Language.apply(",'确认键'修改小票号");
				break;	
			case StatusType.MN_SALEHC:
				help_message += Language.apply(",'确认键'进行红冲");
				break;
			case StatusType.MN_BACKSALE:
				help_message += Language.apply(",'确认键'进行退货");
				break;
		}
    ...
    public void keyReleased(KeyEvent e,int key)
    {
    	try
    	{
    		switch (key)
    		{
    			...
    			case GlobalVar.AliBacBill:
    				if (type == StatusType.MN_XSCX || type == StatusType.MN_MODIFYINVNO) dstbs.printSaleTicketAli();
    				break;
    			case GlobalVar.SuNingBacBill:
    				if (type == StatusType.MN_XSCX || type == StatusType.MN_MODIFYINVNO) dstbs.printSaleTicketSuNing();
    				break;
