...
public class GoodsStockQueryEvent
{
	...
	private GoodsStockQueryBS giqbs = null;
	...
	public GoodsStockQueryEvent(GoodsStockQueryForm giqf, StringBuffer sbBarcode, ICallBack callBack)
	{
		...
		giqbs = CustomLocalize.getDefault().createGoodsStockQueryBS();
	...
	private void addGoodsToSaleform()
	{
		try
		{
			if (tabGoods.getItemCount() > 0)
			{
				...
				if (callBack == null)
					return;
				callBack.exec(tableItem.getText(1));
				if (GlobalInfo.sysPara.isMoreSelectQuerygoods == 'N')
				{
					if (giqbs.sbBarcode != null)
						giqbs.sbBarcode.append(tableItem.getText(1));
					// 单选的时候,关闭窗口
	...
	{
		giqbs.getGoodsStockList(combo, txtCode, txtCode2, tabGoods);
	}
/*
