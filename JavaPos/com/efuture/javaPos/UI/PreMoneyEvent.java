...
public class PreMoneyEvent
{
    ...
    PreMoneyBS preBS = null;

    public PreMoneyEvent(PreMoneyForm preForm)
    {
        ...
        preBS = CustomLocalize.getDefault().createPreMoneyBS();
        ...
        // 读取上次输入的备用金
        Vector tableInfo = preBS.getTableInfo();
        TableItem item = null;
        String[] row = null;
        
        if (tableInfo.size() <= 0)
        {
        	preForm.shell.dispose();
    ...
    public boolean saveData()
    {
 		try
 		{
            ...
            {
                prePay    = new String[2];
                prePay[0] = items[i].getText(0);
                prePay[1] = items[i].getText(1);
                ...
                tableInfo.add(prePay);
            }

            //保存备用金信息
            return preBS.savePreMoney(tableInfo);
 		}
        catch (Exception er)
        {
            er.printStackTrace();
            new MessageBox(Language.apply("备用金保存出现异常!\n请确定输入的金额是否合法!"), null, false);
            return false;
        }           	
    }
}
