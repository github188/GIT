...
public class MutiSelectEvent
{
    private MutiSelectForm form = null;
    ...
    private MutiSelectBS msbs = null;

	public MutiSelectEvent(final MutiSelectForm form, String help_txt, String[] title,int[] width, Vector content,boolean textInput,boolean manychoice,boolean modifyvalue,int rowindex,boolean specifyback,String[] title1,int[] width1,Vector content2,int funcID,boolean cannotchoice)
    {
        this.form = form;
        ...
        form.table.addMouseListener(new MouseAdapter()
        {
            ...
            {
            	if (MutiSelectEvent.this.textInput && form.table.getSelectionIndex() >= 0)
            	{
        ...
        //msbs = CustomLocalize.getDefault().createMutiSelectBS();
        msbs = new MutiSelectBS();
        
        if (specifyback) key.inputMode = key.DoubleInput;
        
        Rectangle rec = Display.getCurrent().getPrimaryMonitor().getClientArea();
        form.shell.setBounds((rec.width - form.shell.getSize().x) / 2,
                             (rec.height - form.shell.getSize().y) / 2,
        ...
        msbs.initBS(this, form, funcID, content2, cannotchoice, cannotchoice, cannotchoice, 0, cannotchoice, cannotchoice);
    }
                		...
                		if (!form.text.getText().trim().equals(""))
                		{
                			if (specifyback)
                            {
                				if (row[3] != null && !row[3].trim().equals(""))
                				{
	                				...
	                				{
	                					new MessageBox("当前输入的退货数量大于原数量,请重新输入!");
	                					form.text.selectAll();
	                					return;
	                				}
	                				...
	                				{
	                					new MessageBox("当前输入的退货数量不能小于1,请重新输入!");
	                					form.text.selectAll();
	                					return;
	                				}
                				}
