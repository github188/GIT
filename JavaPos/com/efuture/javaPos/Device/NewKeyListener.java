...
public class NewKeyListener implements KeyListener
{
	...
	public final int MsrKeyInputICOUT = 5; // 不允许使用IC卡
	...
	public void keyPressed(KeyEvent e)
	{
		...
		if (showABC && !((keyValue >= GlobalVar.Key0) && (keyValue <= GlobalVar.Decimal)) && keyValue != GlobalVar.Plus && keyValue != GlobalVar.Minu && keyValue != GlobalVar.Mul && keyValue != GlobalVar.Div && keyValue != GlobalVar.BkSp)
		{
			e.doit = true;
		}
	...
	public void keyReleasedDoEvent(KeyEvent e)
	{
		...
		if (!funcRun && !funcRun_ext)
		{
			...
			// 执行功能键
			if (!showABC && TaskExecute.getDefault().executeFuncKey(keyValue))
			{
				bContinue = false;
			}
		...
		if (event != null)
		{
			...
			else
			{
				...
				if ((curInputMode != MsrKeyInputICOUT ) && (keyValue == GlobalVar.Enter) && GlobalInfo.isStartICCard && (NewKeyListener.searchKeyCode(GlobalVar.ICInput) > 0) && (ICCard.getDefault() != null)&&GlobalInfo.sysPara.icCardIsEnter.equals("Y"))
				{
					// 初始化IC卡卡号和金额
				...
				if ((keyValue == GlobalVar.BankTracker) && GlobalInfo.isStartBankTracker && (NewKeyListener.searchKeyCode(GlobalVar.BankTracker) > 0) && (BankTracker.getDefault() != null))
				{
	...
	private boolean TextControlDo(Text txt, KeyEvent e)
	{
		...
		//在文本框中，扫描仪，读卡器读入一些不必要字符时，过滤掉
		if (GlobalInfo.sysPara != null 
				&& GlobalInfo.sysPara.textFilterChar != null 
				&& !GlobalInfo.sysPara.textFilterChar.equals("") 
				&& GlobalInfo.sysPara.textFilterChar.indexOf( (char)e.keyCode) > -1)
		{
			return true;
		}
	...
	private boolean MsrInputDo(Text txt, KeyEvent e)
	{
		...
		PosLog.getLog(getClass()).debug(Language.apply("键值：") + keyValue);
		// 读到回车表示本次刷卡数据已完成
