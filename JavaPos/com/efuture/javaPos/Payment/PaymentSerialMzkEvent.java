...
public class PaymentSerialMzkEvent
{
    ...
    private PaymentSerialMzk payObj = null; 

    public PaymentSerialMzkEvent(PaymentSerialMzkForm form, PaymentSerialMzk pay, SaleBS sale, boolean isallowinputpaymoney)
    {
    	saleBS = sale;
        payObj = pay;
    	...
    	else if (txt.equals(textCardNoEnd))
    	{
            ...
            if (payObj.findMzk(textCardNoStart.getText(),textCardNoEnd.getText()))
            {
            	for(int i = 0;i<payObj.arrMzkRets.size();i++)
            	{
                	...
                	{
                		sttMemo.setText("[" + payObj.mzkret.cardno + "]" + Language.apply("此卡已付，请先删除原付款"));
                		return;
                	}
            	}
