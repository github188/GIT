...
public class PaymentCoupon extends PaymentMzk
{
	...
	public PaymentCoupon(PayModeDef mode, SaleBS sale)
	{
		initPayment(mode, sale);
	}

	public PaymentCoupon(SalePayDef pay, SaleHeadDef head)
	{
	...
	public boolean paymentApportion(SalePayDef spay, Payment payobj, boolean test)
	{
		...
		for (int i = 0; i < v.size(); i++)
		{
			...
			double spje = ManipulatePrecision.doubleConvert(Math.min(syje, je6));
	...
	public boolean initList()
	{
		try
		{
			...
			{
				new MessageBox(Language.apply("当前没有有效券余额\n或\n此券已经消费或者过期"));
			}
		}
		catch (Exception er)
		{
	...
	public String getValidJe(int index)
	{
		...
		if (SellType.ISSALE(salehead.djlb))
		{
			...
			// line = "可收" + rows[1] + "金额为： " +
			// ManipulatePrecision.doubleToString(allowpayje);
				...
				for (i = 0; i < lines.length; i++)
				{
					...
					if (l.indexOf(",") > 0)
					{
						...
						if (cid.equals(rows[0]))
						{
							...
							// line += "汇率为 " +
							// ManipulatePrecision.doubleToString(hl);
							line = Language.apply("汇率为 {0}", new Object[] { ManipulatePrecision.doubleToString(hl) });
							break;
						}
					}
