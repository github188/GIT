...
public class JfQueryInfoBS
{
	protected CustomerDef cust = null;
	protected PaymentJfNew jf = null;
	protected Vector jfList = null;
	protected ArrayList jfDetail = null;
	protected HykInfoQueryBS hyk = null;
	protected int jfTypeChoice = -1;
	...
	public boolean findHYK(String track1, String track2, String track3)
	{
		...
		if (jf.findFjk(track1, s[1], track3) && jf.initList())
		{
			jfList = jf.couponList;
		}
		...
		if (jfList != null)
		{
			...
			{
				String[] row = (String[]) jfList.elementAt(i);
		...
		if (zje > 0)
		{
			...
			if (jf.findJfInfo(track1, jf.mzkret.cardno, track3, list))
			{
				if (list.size() > 0)
				{
					jfDetail = list;
	...
	{
		if ((jfList == null) || (jfList.size() <= 0)) { return; }

		String[] title = { Language.apply("积分名称"), Language.apply("积分余额") };
		int[] width = { 100, 100 };
	...
	public void displayCouponDetail(PosTable table)
	{
		if (jfDetail != null)
		{
			String[] title = { Language.apply("积分名称"), Language.apply("积分开始日期"), Language.apply("积分结束日期"), Language.apply("积分余额") };
			...
			for (int i = 0; i < jfDetail.size(); i++)
			{
				FjkInfoDef fjd = (FjkInfoDef) jfDetail.get(i);
	...
	public void specialDeal(JfQueryInfoEvent event)
	{
	}
}
