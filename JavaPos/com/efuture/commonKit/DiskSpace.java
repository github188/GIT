...
public class DiskSpace {
	...
	public static long os_freesize(String dirName) {
		String[] cmds = null;
		long freeSize = -1;
		int osType = os_type();
		switch (osType) {
	...
	private static long os_freesize_unix(String s) {
		...
		_log.debug(Language.apply("os_freesize_unix() 目录:\t") + items[0]);
		_log.debug(Language.apply("os_freesize_unix() 总共:\t") + items[1]);
		_log.debug(Language.apply("os_freesize_unix() 已用:\t") + items[2]);
		_log.debug(Language.apply("os_freesize_unix() 可用:\t") + items[3]);
		_log.debug(Language.apply("os_freesize_unix() 可用%:\t") + items[4]);
		_log.debug(Language.apply("os_freesize_unix() 挂接:\t") + items[5]);
		if (items[3] == null) {
			_log.warn("(ss[3]==null)");
			return -1;
		}
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length == 0) {
			...
			for (int i = 0; i < args.length; i++) {
				_log.info(args[i] + Language.apply(" 剩余空间（B）:") + getFreeDiskSpace(args[i]));
			}
		}
	}
}
