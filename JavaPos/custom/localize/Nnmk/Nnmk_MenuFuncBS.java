...
public class Nnmk_MenuFuncBS extends MenuFuncBS
{
	...
	public boolean execExtendFuncMenu(MenuFuncDef mfd, MenuFuncEvent mffe)
	{
		...
		else if (Integer.parseInt(mfd.code) == StatusType.MN_FJKXX)
		{
			code = mfd.code;
			openFjkXx(mfd, mffe);
			return true;
		}
		else if (Convert.toInt(mfd.code) == 399)
		{
			Nnmk_ExchangeCardForm a = new Nnmk_ExchangeCardForm();
			a.open();
			return true;
		}
		else if (Convert.toInt(mfd.code) == 414 || Convert.toInt(mfd.code) == 415 || Convert.toInt(mfd.code) == 416)
		{
			try
			{
