...
public class Zmsy_NewKeyListener_RPT extends Zmjc_NewKeyListener
{
	...
	public static int searchKey(int key)
	{
		...
		for (int i = 0; i < validnum; i++)
		{
			...
			{ 
				...
				if (k==GlobalVar.Enter || k==GlobalVar.Exit  || k==GlobalVar.Quantity  || k==GlobalVar.Pay || k==GlobalVar.ArrowUp || k==GlobalVar.ArrowDown)
				{
	...
	public void keyReleasedDoEvent(KeyEvent e)
	{
		...
		if (event != null)
		{
			...
			else
			{
				...
				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartICCard && (Zmsy_NewKeyListener_RPT.searchKeyCode(GlobalVar.ICInput) > 0) && (ICCard.getDefault() != null))
				{
					// 初始化IC卡卡号和金额
					...
					{
						msrfinish = false;
						Zmsy_NewKeyListener_RPT.sendKey(GlobalVar.ICInput);
					}
				}
				
				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartBankTracker && (Zmsy_NewKeyListener_RPT.searchKeyCode(GlobalVar.BankTracker) > 0) && ( BankTracker.getDefault() != null))
				{
					...
					{
						msrfinish = false;
						Zmsy_NewKeyListener_RPT.sendKey(GlobalVar.BankTracker);
					}
				}
		...
		try
		{
			...
			if (control.getClass().getName().equals("org.eclipse.swt.widgets.Text") && (control.getData() != null) && ((String) control.getData()).equals("MSRINPUT"))
			{
				if (Display.getCurrent() != null)
				{
					...
					((Text) control).setText("请将操作转向刷卡设备...");
					((Text) control).update();
		...
		try
		{
			...
			if (control.getClass().getName().equals("org.eclipse.swt.widgets.Text") && (control.getData() != null) && ((String) control.getData()).equals("MSRINPUT"))
			{
				if (Display.getCurrent() != null)
				{
					...
					((Text) control).setText("正在读取IC卡,请等待...");
					((Text) control).update();
