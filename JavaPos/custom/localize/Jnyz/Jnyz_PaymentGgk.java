...
public class Jnyz_PaymentGgk extends Bcrm_PaymentMzk{
...
//		根据不同键盘驱动解析磁道
	...
	public SalePayDef inputPay(String money)
	{
		try
		{
			...
			// 打开明细输入窗口
			new Jnyz_PaymentGgkForm().open(this,saleBS);
...
//		if(track2.trim().length() != 18)
//			{
//				new MessageBox("长度必须为18！"); return false;
//			}
//		
		// 设置请求数据
		setRequestDataByFind(track1,track2,track3);
	...
	public boolean createSalePay(String money)
	{
		if(super.createSalePay(money)){
			/*if(!salehead.djlb.equals(SellType.RETAIL_BACK) || !salehead.djlb.equals(SellType.BATCH_BACK)){
				...
				double syje = ManipulatePrecision.doubleConvert(salehead.ysje - salehead.sjfk,1,1);//剩余金额
...
//	密码输入次数限制验证
	...
	public String getDisplayStatusInfo()
	{
		...
		try
		{
			...
			if (!checkMzkIsBackMoney())
			{
				...
				/*// 计算并显示付款限制
				if (!this.allowpayjealready) this.allowpayje = ManipulatePrecision.doubleConvert(calcPayRuleMaxMoney() / paymode.hl);
				if (this.allowpayje >= 0 && ye >= 0) this.allowpayje = Math.min(allowpayje,ye);
				else if (ye >= 0) this.allowpayje = ye;
	...
	public void specialDeal (Jnyz_PaymentGgkEvent event)
	{
	}
	
	public void setMoneyVisible(Jnyz_PaymentGgkEvent paymentMzkEvent)
	{
	}
	
	public void setPwdAndYe (Jnyz_PaymentGgkEvent event, KeyEvent e)
	{
    	if (isPasswdInput())
    	{
    		// 显示密码
	...
	public void doAfterFail(Jnyz_PaymentGgkEvent mzkEvent)
	{
