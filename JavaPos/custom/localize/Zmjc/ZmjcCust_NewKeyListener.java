...
public class ZmjcCust_NewKeyListener implements KeyListener
{
	...
	public static int searchKey(int key)
	{
		...
		for (int i = 0; i < validnum; i++)
		{
			...
			{ 
				...
				if (k==GlobalVar.Enter || k==GlobalVar.Exit  || k==GlobalVar.Quantity  || k==GlobalVar.Pay  || k==GlobalVar.ArrowDown
						|| k==GlobalVar.ArrowUp   || k==GlobalVar.ArrowLeft   || k==GlobalVar.ArrowRight)
				{
	...
	public void keyReleasedDoEvent(KeyEvent e)
	{
		...
		if (event != null)
		{
			...
			else
			{
				...
				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartICCard && (ZmjcCust_NewKeyListener.searchKeyCode(GlobalVar.ICInput) > 0) && (ICCard.getDefault() != null))
				{
					// 初始化IC卡卡号和金额
					...
					{
						msrfinish = false;
						ZmjcCust_NewKeyListener.sendKey(GlobalVar.ICInput);
					}
				}
				
				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartBankTracker && (ZmjcCust_NewKeyListener.searchKeyCode(GlobalVar.BankTracker) > 0) && ( BankTracker.getDefault() != null))
				{
					...
					{
						msrfinish = false;
						ZmjcCust_NewKeyListener.sendKey(GlobalVar.BankTracker);
					}
				}
