...
public class Zmjc_PaymentDetailEvent
{
    ...
    private Zmjc_PaymentDetail payObj = null;
    private boolean isNeedAccnt = false;
    private boolean isNeedIdno = false;

    public Zmjc_PaymentDetailEvent(Zmjc_PaymentDetailForm form, Zmjc_PaymentDetail pay, SaleBS sale,boolean isneedaccnt,boolean isneedidno)
    {
        this.saleBS = sale;
        this.payObj = pay;
        ...
        NewKeyEvent event = new NewKeyEvent()
        {
            ...
            {   
            	if (track2!=null && track2.length()>0) track2 = track2.replace(":","").trim();
            	// 将卡号解析出来作为文本输入
            	if (track2.indexOf('=') >= 0)
            	{
        ...
        ZmjcPayDetail_NewKeyListener key = new ZmjcPayDetail_NewKeyListener();
        key.event   = event;
