...
public class ZmjcPayDetail_NewKeyListener implements KeyListener
{
	...
	public final int MsrRetTracks = 100; //直接返回轨道信息且刷卡、键盘输入皆可模式且手工输入*号显示
	...
	//过虑回车数
	private static int enterNum= GlobalInfo.sysPara.enterNum<0 ? 0 : GlobalInfo.sysPara.enterNum;
	private int enterNum_tmp=0;
	...
	public void keyPressed(KeyEvent e)
	{
		...
		if (e.widget.getClass().getName().equals("org.eclipse.swt.widgets.Text"))
		{
			...
			PosLog.getLog(getClass()).debug("--keyReleased-- txt.getText= " + txt.getText());
			if (isMsrKeyboard && this.trackbuffer.length()>0 && ((keyValue == GlobalVar.Enter) || (e.character == '\r') || (e.character == '\n'))) 
			{
				if (txt.getText().length()>0 && (!txt.getText().endsWith("?") && !txt.getText().endsWith("/")))
				{
					//当时手输模式时(不以?/等字符结尾的),只用一个回车即可
					enterNum_tmp=1;
				}
				
				if (enterNum_tmp>=enterNum)
				{
					//enterNum_tmp=0;
				}
				else
				{
					//enterNum_tmp++;
					PosLog.getLog(getClass()).debug("--keyReleased-- return " + enterNum_tmp);
					e.doit = false;
					return;
				}
	...
	public void keyReleasedDoEvent(KeyEvent e)
	{
		...
		if (event != null)
		{
			...
			else
			{
				...
				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartICCard && (ZmjcPayDetail_NewKeyListener.searchKeyCode(GlobalVar.ICInput) > 0) && (ICCard.getDefault() != null))
				{
					// 初始化IC卡卡号和金额
					...
					{
						msrfinish = false;
						ZmjcPayDetail_NewKeyListener.sendKey(GlobalVar.ICInput);
					}
				}

				if ((keyValue == GlobalVar.Enter) && GlobalInfo.isStartBankTracker && (ZmjcPayDetail_NewKeyListener.searchKeyCode(GlobalVar.BankTracker) > 0) && (BankTracker.getDefault() != null))
				{
					...
					{
						msrfinish = false;
						ZmjcPayDetail_NewKeyListener.sendKey(GlobalVar.BankTracker);
					}
				}
	...
	private boolean MsrInputDo(Text txt, KeyEvent e)
	{
		...
		//PosLog.getLog(getClass()).debug("==TextControl Text:" + ((Text)e.widget).getText() + ",length=" + trackbuffer.length() + "," + trackbuffer.toString());
		// 读到回车表示本次刷卡数据已完成
		if ((keyValue == GlobalVar.Enter) || (e.character == '\r') || (e.character == '\n'))
		{
			...
			//第三方CRM接口需直接返回轨道数据自己去解析
			//一定要与MsrRetTracks方式配套使用
			if (inputMode  == MsrRetTracks)
			{
				track1 = "";
				track2 = trackbuffer.toString();
				track3 = "";
			}
			else
			{
				...
				else
				{
					...
					if (trackcount>=2 && trackbuffer.charAt(0)!=';') trackcount = 0;//add by 2013.12.09
		...
		// 刷卡、键盘输入皆可的模式
		if ((inputMode == MsrKeyInput) || (inputMode == MsrKeyInputMarker) ||  (inputMode == MsrRetTracks))
		{
