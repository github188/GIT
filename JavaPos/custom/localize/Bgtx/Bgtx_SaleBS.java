...
import custom.localize.Sxtx.Sxtx_NetService;

public class Bgtx_SaleBS extends Cmls_SaleBS
{
	...
	{
		inGoods = goodsDef;
		if(Convert.toDouble(pricestr) == 0.0)   //不定价商品
		{
...
/*    	// 重算所有商品应收
	...
	public void calcVIPZK(int index)
	{
		...
		if (((Bgtx_DataService) DataService.getDefault()).findHYZK(popDef, saleGoodsDef.code, curCustomer.type, saleGoodsDef.gz, saleGoodsDef.catid,
...
//	会员授权
	public boolean memberGrant()
	{
		...
		int[] width = {500};
		Vector v = new Vector();
		v.add(new String[]{"会员卡"});
		v.add(new String[]{"手机号"});
		v.add(new String[]{"联名卡"});
...
//	 获取退货小票信息
		...
		if (((Label) clist.get("lbl_barcode")).getText().equals("商品金额"))
		{
			if (saleGoods.size() > 0)
			{
				...
				if (inGoods.barcode.equals(lastgoods.barcode))
				{
					...
					if(instr_price.trim().length()<=0)
					{
						new MessageBox("请输入商品价格");
	...
	public void paySell()
	{
		...
		//未刷会员卡不提示是否积分
//		if ((saleHead.hykh.trim().length()>0) && (new MessageBox("此单小票是否积分?",null,true).verify() == GlobalVar.Key1))
//		{
//			saleHead.str1 = "1";
//		}
//		else
...
//			goodsCmPop.add(null);
//        
//		if (goods != null && info != null &&Bgtx_CustomLocalize.crmMode())
//		{
