...
public class Cmjb_MenuFuncBS extends MenuFuncBS
{
	...
	private void openQtCx(MenuFuncDef mfd, MenuFuncEvent mffe)
	{
		if (PathFile.fileExist(GlobalVar.HomeBase + "\\posreport\\posreport.exe"))
		{
			try
			{
				CommonMethod.waitForExec(GlobalVar.HomeBase + "\\posreport\\posreport.exe " + printButtonHandle + "," + GlobalInfo.sysPara.mktcode + "," + GlobalInfo.posLogin.gh, "posreport.exe");
			}
			catch (Exception e)
			{
				e.printStackTrace();
