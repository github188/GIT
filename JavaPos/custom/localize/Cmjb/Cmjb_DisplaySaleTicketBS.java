...
public class Cmjb_DisplaySaleTicketBS extends DisplaySaleTicketBS
{
	...
	public void printSaleBill(SaleHeadDef vsalehead, Vector vsalegoods, Vector vsalepay, boolean isRed, boolean enableLHBillMode)
	{
		...
		try
		{
			...
			if (enableLHBillMode)
				Cmjb_LHSaleBillMode.getInstance(vsalehead.djlb).setSaleTicketMSInfo(vsalehead, gifts);
			else
				SaleBillMode.getDefault().setSaleTicketMSInfo(vsalehead, gifts);

			// 检查是否需要重打印赠品联授权
			boolean bok = true;
			if (vsalehead.printnum > 0 && (enableLHBillMode ? Cmjb_LHSaleBillMode.getInstance().needMSInfoPrintGrant() : SaleBillMode.getDefault().needMSInfoPrintGrant()))
			{
			...
			{
				if (enableLHBillMode)
					Cmjb_LHSaleBillMode.getInstance().setSaleTicketMSInfo(vsalehead, null);
				else
		...
		try
		{
			if (vsalehead != null && vsalegoods != null && vsalepay != null)
			{
				if (enableLHBillMode)
				{
					Cmjb_LHSaleBillMode.getInstance().setTemplateObject(vsalehead, vsalegoods, vsalepay);
					Cmjb_LHSaleBillMode.getInstance().printBill();
				}
				else
				{
