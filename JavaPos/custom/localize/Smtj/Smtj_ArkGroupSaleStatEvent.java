...
public class Smtj_ArkGroupSaleStatEvent
{
	...
	public Smtj_ArkGroupSaleStatEvent(Smtj_ArkGroupSaleStatForm agssf)
	{
		...
		//显示功能提示
		GlobalInfo.statusBar.setHelpMessage("按 '打印键' 打印柜组对账单");
    	...
    	try
		{
			switch(key)
			{
				...
				case GlobalVar.Print:
					if(currow < 0)
                	{
                		new MessageBox("没有查询到柜组统计数据,不能打印!", null, false);
                		break;
                	}
