...
public class Ywjb_SaleBS extends Bcrm_SaleBS
{
    ...
    {
    	if (GlobalInfo.sysPara.ispregetmsinfo == 'N')
    	{
    		return true;
    	}
    ...
    public void findGoodsCRMPop(SaleGoodsDef sg, GoodsDef goods, SpareInfoDef info)
    {
        ...
        ((Bcrm_DataService) DataService.getDefault()).findPopRuleCRM(popDef, sg.code,
                                                                      sg.gz, sg.uid,
                                                                      goods.specinfo,
                                                                      sg.catid,
                                                                      sg.ppcode,
                                                                      saleHead.rqsj,
                                                                      cardno, cardtype,saletype);
	    if (popDef.yhspace == 0)
		{
            popDef.yhspace = 0;
            popDef.memo = "";
   ...
   //     new MessageBox(String.valueOf(goodsDef.hyj));
                    ...
                    else
                    {
                            ...
                            if (crc.equals("C"))
                            {
                                ...
                                if (!AccessDayDB.getDefault().updateBankLog(bld))
                                {
                                    ...
                                    AccessDayDB.getDefault()
                                               .writeWorkLog("付款导入" + pay.paycode + " 名称:" + pay.payname + " 金额:" + pay.je + " 类型:消费" + " 卡号:" +
                                                             pay.payno, "0300");
                                }
                            ...
                            else if (crc.equals("D"))
                            {
                                ...
                                if (exsit)
                                {
                                    ...
                                    if (!AccessDayDB.getDefault().updateBankLog(bld))
                                    {
                                        new MessageBox("导入异常付款日志失败");
                                        AccessDayDB.getDefault()
                                                   .writeWorkLog("付款导入" + pay.paycode + " 名称:" + pay.payname + " 金额:" + pay.je + " 类型:退货" + " 卡号:" +
                                                                 pay.payno, "0300");
                                    }
                                }
                        ...
                        else if (verity == GlobalVar.Key2)
                        {
                            ...
                            if (!AccessDayDB.getDefault().updateBankLog(bld))
                            {
                                new MessageBox("导入异常付款日志失败");
                                AccessDayDB.getDefault()
                                           .writeWorkLog("付款导入" + pay.paycode + " 名称:" + pay.payname + " 金额:" + pay.je + " 类型:"+crc + " 卡号:" +
                                                         pay.payno, "0300");
                            }
                            del = true;
                ...
                if ((nwhile >= 2) && havepaycw)
                {
                	...
                	else if ( ret == GlobalVar.Key1)
                    {
                        ...
                        /*
                                                ...
                                                {
                                                    SaleGoodsDef saledef = (SaleGoodsDef) saleGoods.elementAt(x);
                        
                                                    if (saledef.str2.trim().length() > 0)
                                                    {
                                                        //计算除券金额
                                                        String[] str = saledef.str2.split(",");
                                                        double num1 = 0;
                        
                                                        for (int y = 0; y < str.length; y++)
                                                        {
                                                            if ((str[y] != null) && (str[y].length() > 0))
                                                            {
                                                                String[] str1 = str[y].split(":");
                                                                double num2 = 0;
                                                        ...
                                                        String line1 = Convert.appendStringSize("", saledef.name, 1, 20, 60);
                                                        line1 = Convert.appendStringSize(line1, "可参与促销金额为", 21, 19, 60);
                                                        line1 = Convert.appendStringSize(line1,
    ...
    public boolean isSamePop(SaleGoodsDef salegoods1, GoodsDef goods1, GoodsPopDef popDef1, String mjrule1, SaleGoodsDef salegoods2, GoodsDef goods2,
                             GoodsPopDef popDef2, String mjrule2)
    {
        if ((popDef1.memo.indexOf(",") == 0) || (popDef2.memo.indexOf(",") == 0))
        {
            return false;
        }
        ...
        if (mjdh1.equalsIgnoreCase(mjdh2) &&
                ((popDef1.ppcode.equalsIgnoreCase("Y") && popDef2.ppcode.equalsIgnoreCase("Y")) ||
                    ((!popDef1.ppcode.equalsIgnoreCase("Y") || !popDef2.ppcode.equalsIgnoreCase("Y")) &&
                        salegoods1.gz.equalsIgnoreCase(salegoods2.gz))))
        {
            return true;
        }
        else
    ...
    public boolean findKeyValue(SaleGoodsDef sg, GoodsDef goods, SpareInfoDef info)
    {
        ...
        while (true)
        {
            ...
            if (!txt.open("本商品必须输入KEY值才能销售", "Key", "请输入KEY值", buff, 0, 0, false, TextBox.IntegerInput))
            {
                return false;
            }
            ...
            if (!((Ywjb_NetService) NetService.getDefault()).findKeyValue(area, goods.code, sg.gz, buff.toString()))
            {
                continue;
            }
            ...
            if (!txt.open("请输入KEY对应的商品金额", "金额", "金额范围 (" + bottom + " , " + top + ")", buffer, bottom1, top1, true, TextBox.DoubleInput, -1,
                              "金额范围 (" + bottom1 + " , " + top1 + ")", -1))
            {
                continue;
            }
    ...
    public boolean checkGoodsRebate(GoodsDef goodsDef)
    {
        ...
        {
            //判断是否为红标商品
            if ((goodsDef.str2.trim().length() > 0) && (goodsDef.str2.trim().charAt(0) == 'Y'))
            {
                return false;
            }
