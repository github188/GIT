...
public class Ywjb_PaymentZZQ extends PaymentMzk
{
	public Ywjb_PaymentZZQ()
	{}
	
    public Ywjb_PaymentZZQ(PayModeDef mode, SaleBS sale)
    {
        initPayment(mode, sale);
    }

    public Ywjb_PaymentZZQ(SalePayDef pay, SaleHeadDef head)
    {
    ...
    public boolean sendMzkSale(MzkRequestDef req, MzkResultDef ret)
    {
        ...
        {
        	return NetService.getDefault().sendFjkSale(NetService.getDefault().getMemCardHttp(11),req, ret);
        }
        else
        {
            new MessageBox("返券卡交易必须联网使用!");
    ...
    public String getFjkPayTotal(Vector salepay)
    {
        ...
        for (int i = 0; i < salepay.size(); i++)
        {
            ...
            if (((Ywjb_CreatePayment) CreatePayment.getDefault()).isPaymentZZQ(spd.paycode))
            {
