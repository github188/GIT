...
public class Ywjb_PaymentBank extends PaymentBank {
	public Ywjb_PaymentBank()
	{
		super();
	}
	
	public Ywjb_PaymentBank(PayModeDef mode,SaleBS sale)
	{
		super(mode,sale);
	}
	
	public Ywjb_PaymentBank(SalePayDef pay,SaleHeadDef head)
	{
		super(pay,head);
	}
	...
	public void accountPay(boolean ret,PaymentBankFunc pbf)
	{
		...
		else
		{
			...
			{
				salepay.bankno = bankinfo.substring(0,3);
			}
			
			if (bankinfo.length() > 0 && pbf.getReplaceBankNameMode())
			{
				if (bankinfo.length() > 3) salepay.payname = bankinfo.substring(3);
