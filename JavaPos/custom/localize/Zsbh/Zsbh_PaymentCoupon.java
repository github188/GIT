...
public class Zsbh_PaymentCoupon extends PaymentCoupon
{
	public Zsbh_PaymentCoupon()
	{
	}

	public Zsbh_PaymentCoupon(PayModeDef mode, SaleBS sale)
	{
		initPayment(mode, sale);
	}

	public Zsbh_PaymentCoupon(SalePayDef pay, SaleHeadDef head)
	{
	 ...
	 * false; } }
	 */
	public boolean deletePayment(int index, PaymentCoupon pcp)
	{
	...
	public boolean CreateNewjPayment(int index, double money, StringBuffer bufferStr)
	{
		try
		{
			...
			if (SellType.ISBACK(saleBS.saletype) && saleBS.refundTotal > 0)
			{
				double ybje = ManipulatePrecision.doubleConvert(money * cpf.paymode.hl, 2, 1);
				if (saleBS.refundPayment.size() == 0)
				{
					if (ybje > saleBS.refundTotal)
					{
						new MessageBox("当前付款金额过大,请重新输入");
						return false;
					}
				}
				else
				{
					double backcouponmoney = 0.0;
					
					for (int i = 0; i < saleBS.refundPayment.size(); i++)
					{
						SalePayDef pay = (SalePayDef) saleBS.refundPayment.get(i);
						backcouponmoney = ManipulatePrecision.doubleConvert(backcouponmoney+(pay.ybje * pay.hl), 2, 1);

						if (ManipulatePrecision.doubleConvert(ybje+backcouponmoney,2,1) > saleBS.refundTotal)
						{
							new MessageBox("当前付款金额过大,请重新输入");
							return false;
						}
	...
	public void addMessage(PaymentCoupon cpf, StringBuffer bufferStr)
	{
	}
	...
	{
		/*
		 * if (msrInputType != -2) return msrInputType; else
		 */
