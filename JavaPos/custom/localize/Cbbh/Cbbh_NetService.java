...
//			new MessageBox(req.track2);
	...
	public Vector getTotalPop(String fphm,String type,Vector popGoods,String custno,String saletype)
	{
		...
		try
		{
			...
			String line1= Transition.ItemDetail(new String[]{GlobalInfo.sysPara.mktcode,type,custno}, new String[]{"mktcode","type","custno"});
...
//		if(!Cbbh_SaleBS.sendBack){return jjlist;}
		...
		Vector inputgoods = (Vector)popSaleGoods.clone();
		...
		try
		{
			...
			String line1= Transition.ItemDetail(new String[]{GlobalInfo.sysPara.mktcode,type,custno,ticketno,yqje}, new String[]{ "mktcode","type", "custno", "ticketno", "yqje"});
			...
			if (result == 0)
			{
				...
				if(goodslist.size() >0)
				{
					...
					//记录日志
					saveloginfo(fphm,aa.getCmdCode(),inputgoods,Cbbh_SaleBS.crmpopgoodsdetail);
...
//	 发送销售小票
	...
	public Vector savepopgoods(SaleHeadDef saleHead,Vector salegoods)
	{
		...
		{			
			...
			cpd.minsj = cgd.jg;
			cpd.sl = cgd.sl;
			//cpd.sjje = ManipulatePrecision.doubleConvert(cgd.sl * cgd.jg, 2, 1);
			cpd.sjje = cgd.num13; //送往的sjje的获取的sjje
			...
			if(cpd.sl==0)
				...
				cpd.cjj = ManipulatePrecision.doubleConvert(cpd.cjje / cpd.sl,2,1);//成交价
			*/
			
			cpd.cjje = cgd.hjje - cpd.gdpopzk - cpd.rulepopzk - cpd.zdrulepopzk - cpd.lszk;//成交金额
			cpd.cjj = cgd.num12;
	...
	public boolean getBackSaleInfo(String syjh, String fphm, SaleHeadDef shd, Vector saleDetailList, Vector payDetail)
	{
		...
		try
		{
			...
			v = new XmlParse(line.toString()).parseMeth(1, CrmPopDetailDef.sendsaleref);
			if (v.size() < 1)
			{
	...
	{		
		...
		{	
			...
			String line1= Transition.ItemDetail(new String[]{GlobalInfo.sysPara.mktcode,type,custno,paycode,payje,payno}, new String[]{ "mktcode","type", "custno", "paycode", "payje","payno"});
	...
	public Vector getCardInfo(String hykh)
	{
		...
		try
		{
			String[] values = {GlobalInfo.sysPara.mktcode,hykh};
			String[] args = {"mktcode","hykh"};
