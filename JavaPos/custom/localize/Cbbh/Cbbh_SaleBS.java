...
//	public static boolean sendBack = true;;//判断此次退货是否调用付款促销
	...
	//初始化商品的微信券用券标志
	public void initCommodMarks(SpareInfoDef spareinfo)
	{
		//int i;
		//for(i = 0;i< this.goodsSpare.size();++ i)
		//{	
		spareinfo.memo3 = Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE;
		spareinfo.memo2 = Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE;
		//}
	}
			...
			for (int j = 0; j < salegoods.size(); j++)
			{
				...
				for(int i =0;i<crmpopgoodsdetail.size();i++)
				{
					...
					if(cpd.rowno == cgd.rowno)
					{
						...
						{
							//目的是为了脱机的时候刷新crmpopgoodsdetail的cjje,cjj
							((CrmPopDetailDef) crmpopgoodsdetail.elementAt(i)).cjje = cpd.sjje - (cpd.gdpopzk + cpd.rulepopzk + cpd.zdrulepopzk + cpd.lszk);//成交金额
							((CrmPopDetailDef) crmpopgoodsdetail.elementAt(i)).cjj = cpd.cjje / cpd.sl;//成交价
						}
...
//	 输入折让金额
	public boolean inputRebatePrice(int index)
	{
		...
		//然后将salegoods保存到crmpopgoodsdetail
		savepopgoods(saleGoods);
...
//	
	...
	//					抹零功能（总折让）
					...
					sgd.num12 = cpdef.cjj;//保存cjj
					sgd.num13 = cpdef.sjje;//保存SJJE
...
//					所用电子券
					...
					//根据后台返回的商品cjje + 商品的合计折扣，反算商品的合计金额,这样为了保证商品的cjje和后台计算结果一致
					sgd.hjje = cpdef.cjje + getZZK(sgd);
				}
			}
		}
...
//	清除新促销临时数据
	...
	public boolean exitPaySell()
	{
		...
		{
			NetService.getDefault().getServerTime(GlobalInfo.localHttp, new TimeDate());
			return true;
		}
...
//    	if(!SellType.ISBACK(saletype) && GlobalInfo.isOnline)
    ...
    public boolean payComplete()
	{
    	if(GlobalInfo.isOnline && !SellType.ISBACK(saletype) || (saleHead.yfphm != null && saleHead.yfphm.length() > 0) && (saleHead.ysyjh != null && saleHead.ysyjh.length() > 0))
    	{
    		...
    		{
    			new MessageBox("检查受限付款方式失败,请取消本笔交易,重新付款！"); 
    			return false;
    		}
    	}
    	else
...
//				crmpopgoodsdetail = poplist;
	...
	public boolean readBrokenData()
	{
		...
		{
			this.saleEvent.setVIPInfo(saleHead.hykh);
