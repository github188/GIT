...
public class Ytpj_DataService extends Bstd_DataService {
		...
		if (SellType.ISSALE(saleHead.djlb)) {//销售  of_MemberSale()
			if (RdPlugins.getDefault().getPlugins1().exec(
				...
				if (bonusAlterReturn != null
					...
					TransComplete(bonusAlterReturn.substring(2, 14));//会员交易达成
			...
			String bonusAlterRequest = mktcode + "," + syyh
					+ "," + track + "," + fphm + ","
					+ UnTraceNum + "," + UnBatchNum + ","
					+ Yjyrq + "," + Yfphm + ",2," + cashAmount
					+ ",0,"
					+ Math.abs(saleHead.bcjf);
			PosLog.getLog(getClass()).info(
					"大会员退货:"+ bonusAlterRequest);
			
			if (RdPlugins.getDefault().getPlugins1().exec(5,
					bonusAlterRequest)) {
				 bonusAlterReturn = (String) RdPlugins
						.getDefault().getPlugins1().getObject();
				if (bonusAlterReturn != null
						&& bonusAlterReturn.trim().length() > 0) {
					/**
					 ...
					 * char CardNumber[19] //
					 * 卡号 char Bonus[12] // 当前积分余额 
					 * char ValidBonus[12] // 当前可用积分 
					 * char ReBonus[12] //
					 * 积分退还 char ReGiveBonus[12] // 积分赠送退还 char
					 * PayAmount[12] // 本交易已支付金额 char
					 * BalanceAmount[12] // 本交易未付余额 char
					 * BonusAmount[12] // 本次可退回积分金额 char
					 * ReturnCash[12] // 退货补现金额
					 */
					PosLog.getLog(getClass()).info(
							"小票号:" + saleHead.fphm + " "
									+ bonusAlterReturn);
					...
					if (bonusAlterReturn.substring(0, 2)
							.equals("00")) {
						TransComplete(bonusAlterReturn.substring(2, 14));
						...
						saleHead.num2 = Convert.toDouble(bonusAlterReturn.substring(51, 63));//累计积分
							...
							.equals("FE")) {
						new MessageBox(
								"泰豆余额不足，请持小票至服务台购买泰豆完成退货！");
						Ytpj_MenuFuncBS wm = new Ytpj_MenuFuncBS();
						wm.PrintDHYDoc();
					} else {
						new MessageBox("大会员积分退货接口返回数据失败："
								+ bonusAlterReturn.substring(0,
										2));
					}
				}
			} else {
