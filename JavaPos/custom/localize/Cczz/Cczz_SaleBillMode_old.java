...
//不上专柜收银请用这个
public class Cczz_SaleBillMode_old extends Bcrm_SaleBillMode
{
	...
	protected String extendCase(PrintTemplateItem item, int index)
	{
		...
		switch (Integer.parseInt(item.code))
		{
			...
			case SBM_inputbarcode:
				...
				{
					if (((SaleGoodsDef) salegoods.elementAt(index)).inputbarcode != null
							&& ((SaleGoodsDef) salegoods.elementAt(index)).inputbarcode.trim().length() > 0)
					{
...
//				((ZZBJ_IBMJPOS_Printer) Printer.getDefault().printer).posPrinter.setSlpLineSpacing(5);
				...
				for (int n = 0; n < newSgds.size(); n++)
				{
					...
					line = Convert.appendStringSize(line, ManipulatePrecision
																				.doubleToString(newSgd.sl * SellType.SELLSIGN(salehead.djlb), 4, 1,
																								true), 24, 4, Width, 0);
...
//				((ZZBJ_IBMJPOS_Printer) Printer.getDefault().printer).posPrinter.setSlpLineSpacing(5);
				...
				Printer.getDefault().printLine_Slip( line2 );
...
//		}
	...
	public void printPopBill()
	{
		try
		{
			...
			for (i = 0; i < salegoods.size(); i++)
			{
				...
				{
					zje += je;
					String line = Convert.appendStringSize("", saleGoodsDef.barcode, 0, 13, 39);
	...
	private void printCarParkInfo()
	{
		if(carParkInfo!=null&&carParkInfo.size()>0){
			...
			printLine("==============停车联==================");
			//printLine("您当日单笔消费满1000元即享当日营业时间内免费停车4小时。办理营业时间内免费停车，"+"须将此联交给停车场收费员，请您妥善保管。");
			BufferedReader br = CommonMethod.readFile(GlobalVar.ConfigPath+"\\carpark.ini","GBK");
			String line = null;
			try
			{
	...
	private void printBKL()
	{
				...
				&& SellType.ISSALE(salehead.djlb) && fje > 0)
		{
			...
			if (GlobalInfo.syjDef.priv == null || GlobalInfo.syjDef.priv.trim().length() == 0)
			{
				...
				line = Convert.appendStringSize(line, String.valueOf(salehead.fphm), 30, 7, 38);
				printLine(line);
				printLine(" 消费金额：" + ManipulatePrecision.doubleToString(fje));
				printLine(Convert.appendStringSize("", "=================================================", 1, 37, 38));
			}
		}
...
//	        	顾客意见联
    ...
    public void printKpInfo()
	{
		if(kpInfo!=null && kpInfo.size()>0){
			for(int i = 0 ;i<kpInfo.size();i++ ) printLine(String.valueOf(kpInfo.elementAt(i)));
		}
	}
