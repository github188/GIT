...
public class Cczz_AccessDayDB extends AccessDayDB {
	...
	public boolean checkSaleData(SaleHeadDef saleHead, Vector saleGoods,Vector salePayment)
	{
		...
		{	
			...
			long fphm1 = 0;
        	if ( (fphm1 = ((Cczz_AccessDayDB)AccessDayDB.getDefault()).getHcHHbackinfo(ConfigClass.CashRegisterCode, String.valueOf(GlobalInfo.syjStatus.fphm - 1))) > 0)
        	{
	...
	public void getBankLogSummary(int[] bs,double[] je)
	{
		ResultSet rs = null;
		
		BankLogDef bcd = new BankLogDef();
		Vector v = new Vector();
		rs = GlobalInfo.dayDB.selectData("select * from BANKLOG where (retbz = 'Y') and (type = '"+PaymentBank.XYKXF+"' or type = '"+PaymentBank.XYKCX+"'or type = '"+PaymentBank.XYKTH+"') order by rowcode");
			
			try {
				while (rs != null && rs.next())
				{
					bcd = new BankLogDef();
					if (!GlobalInfo.dayDB.getResultSetToObject(bcd))
					{
						break;
					}
			...
			{
				bcd = (BankLogDef) v.elementAt(i);
				if (bcd.type.equals(String.valueOf(PaymentBank.XYKXF)))
				{
					xfbs ++;
					xfje += bcd.je;
				}
				else if (bcd.type.equals(String.valueOf(PaymentBank.XYKCX)))
				{
					cxbs ++;
					cxje += bcd.je;
				}
				else if (bcd.type.equals(String.valueOf(PaymentBank.XYKTH)))
				{
					thbs ++;
					thje += bcd.je;
				}
			...
			je[0] = ManipulatePrecision.doubleConvert(xfje);
			je[1] = ManipulatePrecision.doubleConvert(thje+cxje);
	...
	public boolean updateSaleJf(long fphm,int flag,double bcjf,double ljjf,String memo)
    {
        ...
        try
        {
            ...
            {
            	case 1:
            		line = "update SALEHEAD set bcjf = " + bcjf + ",ljjf = " + ljjf + ",num4 = "+memo+" where syjh = '" + ConfigClass.CashRegisterCode + "' and  fphm = " + fphm;
            		break;
            	case 2:
