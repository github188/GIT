...
public class Zspj_PaymentDzjEvent
{
    ...
    public Zspj_PaymentDzj payObj = null;
    public boolean ShellIsDisposed = false;
    
    public Zspj_PaymentDzjEvent(Zspj_PaymentDzjForm form, Zspj_PaymentDzj pay, SaleBS sale)
    {
        this.saleBS = sale;
        this.payObj = pay;
    ...
    public void keyReleased(KeyEvent e, int key)
    {
        ...
        switch (key)
        {
            ...
            case GlobalVar.Exit:
            	try {
//            		 if (PathFile.fileExist(Zspj_PaymentDzj.Path+"\\request.txt"))
// 		            {
            		if(Zspj_PaymentDzj.CancelWriteRequest(null,null))
            		{
		            	 // 调用接口模块
		                if (PathFile.fileExist(Zspj_PaymentDzj.Path+"\\javaposbank.exe"))
		                {
		                	
								CommonMethod.waitForExec(Zspj_PaymentDzj.Path+"\\javaposbank.exe ZSDZJ");
								Zspj_PaymentDzj.CancelReadResult(null,null);
//								ProgressBox progress=  new ProgressBox();
//		            			progress.setText("正在取消交易认证。。。。。。");
							
		                }
		                else
		                {
		                    new MessageBox("取消交易认证失败，找不到付款模块 javaposbank.exe");
		                    return;
		                }
            		}
// 		            }
    ...
    public void recycle()
    {
    	Zspj_PaymentDzj.recycleStatus = !Zspj_PaymentDzj.recycleStatus;
