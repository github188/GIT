...
public class Bstd_SaleBS extends Bstd_SaleBS3Pay
{
	...
	// 计算本笔销售积分,默认能过getsalejf来获得，如果脱网的化不将累计积分复制到本次积分，这样会造成打印积分的问题
	...
	{
		checkIndex = -1;
		GlobalInfo.sysPara.isHbGoods = goodscombineFlag;
		return super.initGui(type);
	}
	...
	{
		if (curCustomer != null && curCustomer.isHandInput && GlobalInfo.sysPara.isusecoinbag == 'N')
		{
			new MessageBox("手工输入会员卡号不允许使用会员相关功能!");
			return false;
		}
