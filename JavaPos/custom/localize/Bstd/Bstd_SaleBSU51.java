...
public class Bstd_SaleBSU51 extends Bstd_SaleBS
{
	...
	public void calcVipMaxsl(int index)
	{
	}
	...
	public boolean doU51CmPop()
	{
		...
		for (int rulepos = 0; rulepos < calcCount + 1; rulepos++)
		{
			...
			Bstd_DataService dataService = new Bstd_DataService();
			if (!dataService.findU51PopBillNo(ruleDef, goodsDef.code, goodsDef.gz, goodsDef.catid, goodsDef.ppcode, goodsDef.uid, "", saleHead.rqsj, cardNo))
			{
				continue;
			}
			...
			if (!dataService.findU51PopRule(ruleReqList, rulePopList, ruleDef) || ruleReqList.size() == 0 || rulePopList.size() == 0)
			{
				continue;
			}
			...
			for (n = 0; n < ((SuperMarketPopRuleDef) ruleReqList.get(0)).jc; n++)
			{
				...
				for (l = 0, j = 0; j < ruleReqList.size(); j++)
				{
					...
					if (ruleReq.presentjs == 1)
					{
						...
						{
							bs = new Double(je / ruleReq.yhlsj).longValue(); // 当 bs == 0 时，说明不满足条件，例如:小票满200减5块，小票总金额为100，则 100/200 取整为0，不满足
						}
						if (l == 1)
							t_minbs = bs;
						else
			...
			for (j = 0; j < rulePopList.size(); j++)
			{
					...
					if (ManipulatePrecision.doubleCompare(mjje, 0, 2) > 0)
					{
						...
						for (l = 0; l < ruleReqList.size(); l++)
						{
							...
							for (k = 0; k < saleGoods.size(); k++)
							{
								...
								// cominfo[k].infostr1[10] != 'Y'*/)
								SaleGoodsDef sg = (SaleGoodsDef) saleGoods.get(k);
								if (isMatchCommod((SuperMarketPopRuleDef) ruleReqList.get(l), k) && !(sg.isPopExc == 'N' || sg.isSMPopCalced == 'A'))// && cominfo[k].infostr1[10]!='Y' )
								{
									// 是否折上折
						...
						// 未分配完的金额分配到金额最大的商品上 ,增加判断  t_maxjerow >= 0 条件，否则当 t_maxjerow = -1时，会抛出异常 for yebk 2014-12-03
						if (ManipulatePrecision.doubleCompare(Math.abs(mjje - t_je), 0, 2) > 0 && t_maxjerow >= 0)
						{
							if (ruleDef.type == '8')
							{
				...
				if (rulePop.yhdjlb == 'C')
				{
					...
					/*
					 * //检查商品个数 if (salecom_num >= MAX_SALECOM) {
					 * NotRuleDjbh.FreeAll(); // delete[] rulecom; return; }
					 */
				...
				if (rulePop.yhdjlb == 'X')
				{
					if (ManipulatePrecision.doubleCompare(zje, ((SuperMarketPopRuleDef) ruleReqList.get(0)).yhlsj, 2) >= 0 && ManipulatePrecision.doubleCompare(((SuperMarketPopRuleDef) ruleReqList.get(0)).yhlsj, 0, 2) > 0)
					{
						...
						if (ManipulatePrecision.doubleCompare(yhje, saleHead.ysje, 2) < 0)
						{
							...
							if (ManipulatePrecision.doubleCompare(Math.abs(yhje - t_je), 0, 2) > 0 && t_maxjerow >= 0)
							{
								if (ruleDef.type == '8')
								{
