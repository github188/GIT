...
public class Nmzd_SaleBillMode extends Cmls_SaleBillMode
{
    ...
    public void printJfxf()
    {
    	...
    	for (int i = 0 ; i < salepay.size(); i++)
    	{
    		...
    		if (sp.paycode.equals("0509") || CreatePayment.getDefault().isPaymentJfxf(sp))
    		{
    			...
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " 扣减积分："+SellType.SELLSIGN(salehead.djlb)*Convert.toDouble(salepaylist[0]), 1, 37, 38));
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " ", 1, 37, 38));
    			...
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " ", 1, 37, 38));
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " ", 1, 37, 38));
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " 本次消费："+ManipulatePrecision.doubleToString(SellType.SELLSIGN(salehead.djlb)*sp.je), 1, 37, 38));
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " ", 1, 37, 38));
    			Printer.getDefault().printLine_Journal(Convert.appendStringSize("", " ", 1, 37, 38));
	...
	protected void printSellBill()
	{
		...
		if (SellType.isJS(salehead.djlb))
		{
			...
			{
				SaleGoodsDef def = (SaleGoodsDef) salegoods.elementAt(i);
				int j = 0;
				for (; j < list.size();j++)
				{
					String[] rows = (String[]) list.elementAt(j);
					if (rows[0].equals(def.inputbarcode))
					{
						rows[1] = rows[1]+i+",";
						break;
					}
			...
			{
				String[] rows = (String[]) list.elementAt(z);
				String[] info = rows[1].substring(0, rows[1].length() - 1).split(",");
		        ...
		        for (int i = 0 ; i < 3 ; i ++)
		        {
			        ...
			        Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "交费单号"+rows[0], 1, 37, 38));
			        if (salegoods.size() > 0 ) Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "费用分店："+((SaleGoodsDef)salegoods.elementAt(Convert.toInt(info[0]))).batch, 1, 37, 38));
			        Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "供应商号:"+((SaleGoodsDef)salegoods.elementAt(Convert.toInt(info[0]))).catid, 1, 37, 38));
			        //ManipulateDateTime mdt  = new ManipulateDateTime();
			        Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "会计时间："+((SaleGoodsDef)salegoods.elementAt(Convert.toInt(info[0]))).fhdd, 1, 37, 38));
			        ...
			        for (int j = 0; j < info.length; j++)
			        {
			        	...
			        	SaleGoodsDef def = (SaleGoodsDef) salegoods.elementAt(Convert.toInt(info[j]));
			        	...
			        	hjje1 = ManipulatePrecision.doubleConvert(hjje1 + def.hjje);
			        }
			        
			        if (i != 1 && i != 2)Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "------------------------------------", 1, 37, 38));
		        
			        Printer.getDefault().printLine_Normal(Convert.appendStringSize("", "费用合计:"+ManipulatePrecision.doubleToString(hjje1*SellType.SELLSIGN(salehead.djlb)), 1, 37, 38));
	...
	//				第一次不打印商品明细
			        ...
			        Printer.getDefault().printLine_Normal("店名:"+ GlobalInfo.sysPara.mktcode);
