...
public class Ybsj_DataService extends Bstd_DataService
{
	public boolean getCustomer(CustomerDef cust, String track)
	{
		return Ybsj_SocketCrmModule.getDefault().queryVipInfo(track, cust);
	}
	...
	public void getCustomerSellJf(SaleHeadDef saleHead)
	{
		...
		if ((saleHead.hykh != null) && (saleHead.hykh.length() > 0))
		{
			...
			{
				if (Ybsj_SocketCrmModule.getDefault().sendVipPoint(saleHead, Convert.toDouble(row[0])))
				{
