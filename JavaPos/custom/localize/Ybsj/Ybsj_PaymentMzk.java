...
public class Ybsj_PaymentMzk extends PaymentMzk
{
	public Ybsj_PaymentMzk()
	{
	}

	public Ybsj_PaymentMzk(PayModeDef mode, SaleBS sale)
	{
		initPayment(mode, sale);
	}

	public Ybsj_PaymentMzk(SalePayDef pay, SaleHeadDef head)
	{
	...
	public boolean findMzkInfo(String track1, String track2, String track3)
	{
		...
		return Ybsj_SocketCrmModule.getDefault().queryMzkInfo(mzkreq, mzkret);
	}
	...
	{
		return Ybsj_SocketCrmModule.getDefault().mzkSale(req, ret);
	}
	...
	public boolean mzkAccount(boolean isAccount)
	{
		...
		do
		{
			...
			if (isAccount)
			{
				...
				// 先获取交易ID，方便写冲正文件
				if (Ybsj_SocketCrmModule.getDefault().mzkPrepareSale(mzkreq) == null)
					return false;
				...
				if (!writeMzkCz())
				{
					...
					// 写冲正文失败，则取消当笔交易
					Ybsj_SocketCrmModule.getDefault().mzkCancelSale(mzkreq);
				...
				// 发送交易请求
				if (!Ybsj_SocketCrmModule.getDefault().mzkConfirmSale(mzkreq))
				{
					if (paynoMsrflag)
					{
						salepay.payno = "";
			...
			else
			{
				...
				if (!Ybsj_SocketCrmModule.getDefault().mzkCancelSale(mzkreq))
					return false;
