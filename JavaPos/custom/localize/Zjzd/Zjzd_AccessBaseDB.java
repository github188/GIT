...
public class Zjzd_AccessBaseDB extends Bcrm_AccessBaseDB {
    public boolean getCustomer(CustomerDef cust, String track) {
        ...
        try {
            ...
            rs = GlobalInfo.baseDB.selectData(
                    "select * from CUSTOMER where track = '" + track + "'");
            ...
            if (rs.next()) {
                ...
                //              查找类型
                rs = GlobalInfo.localDB.selectData(
                        "select * from CUSTOMERTYPE where code = '" +
                        cust.type + "'");
