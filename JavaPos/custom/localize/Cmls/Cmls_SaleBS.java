...
public class Cmls_SaleBS extends Bcrm_SaleBS
{
		...
		else if (checkMemberSale() && curCustomer != null && goodsDef.isvipzk == 'Y' && vipzk && curCustomer.iszk == 'Y')
		{
			...
			if (type == vipzk2 && GlobalInfo.sysPara.vipPromotionCrm != null && GlobalInfo.sysPara.vipPromotionCrm.equals("2"))
			{
				...
				else if (GlobalInfo.sysPara.vipCalcType.equals("1"))
				{
					...
					if (getZZK(saleGoodsDef) > 0 && zszflag && ManipulatePrecision.doubleCompare(saleGoodsDef.hjje - getZZK(saleGoodsDef), saleGoodsDef.hjje * mk, 2) >= 0)
					{
						//vipzsz = ManipulatePrecision.doubleConvert((1 - curCustomer.zkl) * (saleGoodsDef.hjje - getZZK(saleGoodsDef) - fte), 2, 1);
						vipzsz = ManipulatePrecision.doubleConvert((1 - zkl) * (saleGoodsDef.hjje - getZZK(saleGoodsDef) - fte), 2, 1);
					}
			...
			if(popDef.num8!=0)saleGoodsDef.hyzkfd = popDef.num8;
			if( popDef.str6 != null && !popDef.str6.trim().equals("")) saleGoodsDef.hydjbh = popDef.str6;
			// 按价格精度计算折扣
			saleGoodsDef.hyzke = getConvertRebate(index, saleGoodsDef.hyzke);
		}
