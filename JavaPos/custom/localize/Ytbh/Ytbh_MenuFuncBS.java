...
public class Ytbh_MenuFuncBS extends MenuFuncBS {
	public void execFuncMenu(MenuFuncDef mfd, MenuFuncEvent mffe) {
		...
		if (Integer.parseInt(mfd.code) == StatusType.MN_DHY) {
			try {
				...
				if (type.equals("2")) {
					...
					String funcCode = memberFuncReturn
							.substring(3,5);
		...
		} else if (Integer.parseInt(mfd.code) == StatusType.MN_HYK){
			String[] title = { "输入类型" };
			int[] width = { 440 };
			...
			if (track2 == null || track2.equals(""))
				return;
			Ytbh_HykInfoQueryBS bs = new Ytbh_HykInfoQueryBS();
