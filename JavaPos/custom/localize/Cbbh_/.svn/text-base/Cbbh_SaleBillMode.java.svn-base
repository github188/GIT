...
public class Cbbh_SaleBillMode extends Bcrm_SaleBillMode
{
	...
	protected final static int SALEPOPMEMO = 203;//规则促销描述
	protected static final int ZDSALEPOPMEMO = 204;//整单参与促销描述
...
//					new MessageBox("fphm："+String.valueOf(salehead.fphm));
				...
				case SALEPOPMEMO:  //规则促销描述
					
					if(((SaleGoodsDef) salegoods.elementAt(index)).str10 != null)
					{
						String rulememo[] = ((SaleGoodsDef) salegoods.elementAt(index)).str10.split(",");
						if(rulememo.length > 3)
						{
							line = rulememo[3];
						}
						else
				...
				case ZDSALEPOPMEMO:  //整单参与促销描述
					
					String memo = ManipulateStr.getConvertStringNewLine(salehead.str8,32);
				...
				case SBM_syjh: // 收银机号
					...
					if(salehead.syjh != null && salehead.syjh.length() >0)
					{
						line = salehead.syjh;
					}
					else
