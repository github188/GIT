...
import custom.localize.Sfks.Sfks_SaleBillMode;

public class Doug_SaleBillMode extends Bhls_SaleBillMode
{
	...
	public void printBill()
	{
		...
		// 超市或要参数要求打印小票联,才打印
		if ((GlobalInfo.sysPara.fdprintyyy != 'A' || !(GlobalInfo.syjDef.issryyy == 'Y' || (GlobalInfo.syjDef.issryyy == 'A' && !((SaleGoodsDef) salegoods
																																							.elementAt(0)).yyyh
																																												.equals("超市")))))
		{
			// 打印交易小票联
	...
	public void printYYYBill()
	{
		...
		for (i = 0; i < set.size(); i++)
		{
			...
			if (isPagePrint.equals("Y")) Printer.getDefault().setPagePrint_Slip(true, pageLines);

			if (!headTitle.equals(""))
			{
				prnInfo = headTitle.split("\\|");

				if (prnInfo.length > 0 && prnInfo[0].length() > 1) Printer.getDefault().printLine_Slip(prnInfo[0]);

				if (prnInfo.length > 1) Printer.getDefault().printLine_Slip(prnInfo[1]);

				if (prnInfo.length > 2) Printer.getDefault().printLine_Slip(prnInfo[2]);
			}
			else
			{
				Printer.getDefault().printLine_Slip("               DOUGLAS");
				Printer.getDefault().printLine_Slip("**************************************");
				Printer.getDefault().printLine_Slip("欢迎光临DOUGLAS");
			}

			Printer.getDefault().printLine_Slip(salehead.rqsj + "   小票号:" + salehead.fphm);
			...
			String str = "";
			if(SellType.ISBACK(salehead.djlb)){
				str = "(" + SellType.getDefault().typeExchange(salehead.djlb, salehead.hhflag, salehead) + ") " + hjsl + "项合计: " + Convert.appendStringSize("", "-" + ManipulatePrecision.doubleToString(hjje - hjzk), 0, 16, 16, 1);
			}else{
				str = "(" + SellType.getDefault().typeExchange(salehead.djlb, salehead.hhflag, salehead) + ") " + hjsl + "项合计: " + Convert.appendStringSize("", ManipulatePrecision.doubleToString(hjje - hjzk), 0, 16, 16, 1);
			}
			
			Printer.getDefault().printLine_Slip(str);
			if (hjzk > 0) Printer.getDefault().printLine_Slip(
																"总折扣:   "
																		+ Convert.appendStringSize("", ManipulatePrecision.doubleToString(hjzk), 0,
																									10, 10));
			...
			for (j = 0; j < salegoods.size(); j++)
			{
				...
				if (s.length >= 2)
				{
					...
					if (j > lastsaqsp && Double.parseDouble(s[0]) > 0) lastsaqsp = j;
					if (j > lastsbqsp && Double.parseDouble(s[1]) > 0) lastsbqsp = j;
			...
			if (maxaqje <= 0) maxaqje = 1;
			if (maxbqje <= 0) maxbqje = 1;
			...
			{
				spd = (SalePayDef) salepay.elementAt(k);
				if (spd.flag == '2') continue;
				if (!(spd.paycode.equals("0500") && (fjkobj.getFjkPayType(spd).equals("A") || fjkobj.getFjkPayType(spd).equals("B")))) continue;
				...
				if (je > 0) Printer.getDefault().printLine_Slip(
																Convert.appendStringSize("", spd.payname + ": ", 0, 10, 10)
																		+ Convert.appendStringSize("", ManipulatePrecision.doubleToString(je), 0, 10,
																									10, 1));
			}
			...
			for (k = 0; k < salepay.size(); k++)
			{
				...
				else
				{
					...
					double allqfk = ((Double.parseDouble(s[0]) > maxaqje) ? maxaqje : Double.parseDouble(s[0]))
				...
				if (je > 0){
					...
					Printer.getDefault().printLine_Slip(Convert.appendStringSize("", "卡号: " +spd.payno, 0, 30, 30));
			...
			else
			{
				...
				if (allaqyy < 0) allaqyy = 0;
				double allbqyy = Double.parseDouble(s[1]) - maxbqje;
				if (allbqyy < 0) allbqyy = 0;
				double qfk = ((aqpayje > yyyaqje) ? yyyaqje : aqpayje) + ((bqpayje > yyybqje) ? yyybqje : bqpayje);
				double allqfk = ((Double.parseDouble(s[0]) > maxaqje) ? maxaqje : Double.parseDouble(s[0]))
						+ ((Double.parseDouble(s[1]) > maxbqje) ? maxbqje : Double.parseDouble(s[1]));
				double qtyy = ManipulatePrecision.doubleConvert((hjje - hjzk - qfk) / (salehead.ysje - allqfk)
						* (salehead.fk_sysy + salehead.sswr_sysy - allaqyy - allbqyy), 2, 1);
				je = aqyy + bqyy + qtyy;
			...
			yfdyy += je;
			Printer.getDefault().printLine_Slip(
												Convert.appendStringSize("", "收您: ", 0, 10, 10)
														+ Convert.appendStringSize("", ManipulatePrecision.doubleToString(hjpay), 0, 10, 10, 1)
														+ ((Math.abs(je) > 0)
																? "  溢余: "
																		+ Convert.appendStringSize("", ManipulatePrecision.doubleToString(je), 0, 10,
																									10, 1)
			...
			if (salehead.hykh == null) salehead.hykh = "";

			Printer.getDefault().printLine_Slip(
												Convert.appendStringSize("", "会员卡号: ", 0, 10, 10)
														+ Convert.appendStringSize("", salehead.hykh, 0, 30, 30));

			if (!headTitle.equals(""))
			{
				if (prnInfo.length > 3) Printer.getDefault().printLine_Slip(prnInfo[3]);
			}
			else
			{
	...
	public void loadConfig()
	{
		...
		try
		{
			...
			if (customItem.length() > 1 && customItem.indexOf(",") > 0)
			{
				...
				if (tmp.length > 0) isPagePrint = tmp[0].trim();
				if (tmp.length > 1) pageLines = Integer.parseInt(tmp[1].trim());
				if (tmp.length > 2) headTitle = tmp[2];
