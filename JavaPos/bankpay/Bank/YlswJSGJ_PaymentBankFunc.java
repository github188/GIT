...
*西安金沙国际，兴业银行(无界面模式)
*/
public class YlswJSGJ_PaymentBankFunc extends PaymentBankFunc
{	
	...
	public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
            ...
            if (PathFile.fileExist(ConfigClass.BankPath  + "\\receipt.txt"))
            {
                ...
                if (PathFile.fileExist(ConfigClass.BankPath  + "\\receipt.txt"))
                {
            		errmsg = "打印文件receipt.txt无法删除,请重试";
            		XYKSetError("XX",errmsg);
            		new MessageBox(errmsg);
            		return false;   	
                }
...
//			 返回码
			...
			String tracestr = newLine.substring(26, 32);
			if(tracestr.trim().length() > 0)
			{
				// 流水号
				bld.trace = Long.parseLong(tracestr);
			}
			
			// 金额
			String jestr = newLine.substring(32, 44);
			if(jestr.trim().length() > 0)
			{
				bld.je = ManipulatePrecision.mul(Double.parseDouble(jestr), 0.01);
			}
			// 系统参考号
			bld.authno = newLine.substring(123, 135);
			
			if(newLine.length() >= 148)
			{
				// LRC
				String LRC = newLine.substring(145, 148);
