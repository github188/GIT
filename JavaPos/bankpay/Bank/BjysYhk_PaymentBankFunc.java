...
//遵化天润 银行卡
public class BjysYhk_PaymentBankFunc extends PaymentBankFunc {
	class ResponseData {
		...
		public String FkhNo = " ";// 发卡行号
		
		public String PzhNo = " ";//原凭证号
	...
	public boolean XYKExecute(int type, double money, String track1,
		...
		try {
			...
			if (PathFile.fileExist("c:\\dat\\js_yhk.txt")) {
				PathFile.deletePath("c:\\dat\\js_yhk.txt");
			...
			if (PathFile.fileExist("c:\\dat\\javaposbank.exe")) {
				CommonMethod.waitForExec("c:\\dat\\javaposbank.exe TSLX");
			} else {
			...
			if (type == PaymentBank.XYKXF || type == PaymentBank.XYKTH
					|| type == PaymentBank.XYKCX||type == PaymentBank.XYKJZ) {
	...
	public boolean XYKWriteRequest(int type, double money, String track1,
		...
		bld.je = money;
		String jestr = String.valueOf((long) ManipulatePrecision.doubleConvert(
				money * 100, 2, 1));
		...
		case PaymentBank.XYKXF:
			...
			temp = Convert.increaseChar(track2, ' ', 38)
					+ Convert.increaseChar(track3, ' ', 108) + jestr + "      "
					+ "            ";
			break;
		case PaymentBank.XYKCX:
			...
			temp = Convert.increaseChar(track2, ' ', 38)
					+ Convert.increaseChar(track3, ' ', 108) + jestr
					+ Convert.increaseChar(oldseqno, ' ', 6);
			break;
		case PaymentBank.XYKTH:
			...
			temp = Convert.increaseChar(track2, ' ', 38)
					+ Convert.increaseChar(track3, ' ', 108) + jestr + "      "
					+ Convert.increaseChar(oldseqno, ' ', 12)+Convert.increaseChar(olddate, ' ', 6);
			break;
		case PaymentBank.XYKYE: // 查询余额
			type1 = "80";
			// response.TransType = "80";
			temp = Convert.increaseChar(track2, ' ', 38)
					+ Convert.increaseChar(track3, ' ', 108) + "000000000000"
					+ "      " + "            ";
			break;
		case PaymentBank.XYKQD:
			type1 = "91";
			// response.TransType = "91";
			temp = Convert.increaseChar("", ' ', 38)
					+ Convert.increaseChar("", ' ', 108) + "000000000000"
					+ "      " + "            ";
			break;
		...
		line = "01"
				...
				+ date
				+ time
				+ Convert.increaseChar(GlobalInfo.syjStatus.syyh, ' ', 6)
				+ Convert.increaseChar(String
						.valueOf(GlobalInfo.syjStatus.fphm), ' ', 6) + "010000"
		...
		try {
			...
			for (int i = 0; i < GlobalInfo.sysPara.bankprint; i++) {
				...
				try {
					...
					br = CommonMethod.readFileGB2312("c:\\dat\\js_yhk.txt");
	...
	public boolean XYKReadResult() {
		...
		try {
			if (!PathFile.fileExist("C:\\dat\\result.txt")
					|| ((br = CommonMethod.readFileGBK("C:\\dat\\result.txt")) == null)) {
			...
			//
		//	for(int o = 0;o<=line.length()/10;o++){
		//		System.out.println(line.substring(10*o, 10*(o+1)));
		//	}
			bld.type = Convert.newSubString(line, 4, 6).trim();
			response.RetCode = Convert.newSubString(line, 6, 8).trim();
			bld.retcode = response.RetCode;
			bld.retmsg = Convert.newSubString(line, 8, 48).trim();
			response.CardId = Convert.newSubString(line, 71, 90).trim();
			bld.cardno = response.CardId;

			response.PosMid = Convert.newSubString(line, 48, 63).trim();
			response.PosTid = Convert.newSubString(line, 63, 71).trim();
			response.BonusInfo = Convert.newSubString(line, 207, 226).trim();
			response.ExpDate = Convert.newSubString(line, 110, 114).trim();
			response.PosDate = Convert.newSubString(line, 114, 120).trim();
			response.PosTime = Convert.newSubString(line, 120, 126).trim();
			response.BatchNo = Convert.newSubString(line, 126, 132).trim();
			response.VoiceNo = Convert.newSubString(line, 132, 138).trim();
			response.PosJnl = Convert.newSubString(line, 150, 162).trim();
			response.AuthorCode = Convert.newSubString(line, 162, 168).trim();
			response.Amount = Convert.newSubString(line, 138, 150).trim();
			response.FkhNo = Convert.newSubString(line, 90, 98).trim();
			response.PzhNo = Convert.newSubString(line, 303, 309).trim();
		//	response.PzhNo = line.substring(166, 172);
			if(response.RetCode.trim().equals("01")){
				errmsg = "商户号不足";
				return false;
			}
			if (response.PosMid.trim().length()!=15) {
				errmsg = "查询发卡方";
				return false;
			}
			...
			if (bld.type.equals("30") || bld.type.equals("40")
					|| bld.type.equals("60")) {
				String je = Convert.newSubString(line, 138, 150).trim();
	...
	public boolean XYKNeedPrintDoc() {
		...
		String saleJe = "";
		try {
			// saleJe = String.valueOf(Double.parseDouble(response.Amount) /
			// 100);
			...
			String cardNO = response.CardId.substring(0, 9)+"******"+response.CardId.substring(15);
			...
			sb.append("遵化市天之润广场商贸有限公司 \r\n");
			sb.append("终端号(TERMINAL ID)：" + response.PosTid + "\r\n");
			sb.append("-------------------------------\r\n");
			sb.append("卡号(CARD NO.)：" + cardNO + "\r\n");
			sb.append("收单行号："+"14411200"+ "\r\n");
			sb.append("卡有效期(EXP. DATE)：" + response.ExpDate + "\r\n");
			sb.append("发卡行号： " + response.FkhNo + " \r\n");
			...
			if (bld.type.trim().equals("60")) {
				sb.append("交易类型(TRANS.TYPE. )：" + " 退货" + "\r\n");
			}
			if (bld.type.trim().equals("40")) {
				sb.append("交易类型(TRANS.TYPE. )：" + " 撤销" + "\r\n");
				sb.append("原凭证号(OLD VOUCHER )：" + response.PzhNo + "\r\n");
			}
			sb.append("日期时间(DATE/TIME)：" + response.PosDate + " "
					+ response.PosTime + "\r\n");
			...
			sb
					.append("I ACKNOWLEDGE SATISFACTORY RECEIPT OF RELATIVE GOODS/SERVICES\r\n");
*/
			sb.toString();
			if (PathFile.fileExist("c:\\dat\\print.txt"))
				PathFile.deletePath("c:\\dat\\print.txt");
		...
		try {
			...
			for (int i = 0; i < GlobalInfo.sysPara.bankprint; i++) {
				...
				try {
					...
					if(i==0){
						Printer.getDefault().printLine_Journal("持卡人签名(CARDHOLDER SIGNATURE)\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("-------------------------------\r\n");
					}
					Printer.getDefault().printLine_Journal("本人已接受此单据金额的有关商品服务并愿意遵守与有关银行签订的持卡人合约一切规定\r\n");
					Printer.getDefault().printLine_Journal("I ACKNOWLEDGE SATISFACTORY RECEIPT OF RELATIVE GOODS/SERVICES \r\n");
					if(i==0){
						Printer.getDefault().printLine_Journal("商户存根(MERCHANT COPY)\r\n");//持卡人存根(CARD HOLDR COPY)
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
					}else{
						Printer.getDefault().printLine_Journal("持卡人存根(CARD HOLDR COPY)\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
						Printer.getDefault().printLine_Journal("\r\n");
					}
					
					new MessageBox("请撕下银联签购单");
				} catch (Exception e) {
					new MessageBox(e.getMessage());
				} finally {
					if (br != null) {
