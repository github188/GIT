...
//浙江工商银行
public class GsyhZj_PaymentBankFunc extends PaymentBankFunc
{
...
//				 检查交易是否成功
		...
		try
		{
			...
			if (result != null && result.trim().length() > 2)
			{
				...
				{					
					...
					if(result.trim().length()>120)bld.bankinfo = result.substring(100, 120);
					/**
					 * 第1－2位 ，返回码，2bytes，00：成功，其他：失败（详细对照关系请看附件）
					 若返回码为其他，后续内容为错误信息，不定长；
					 若返回码为00，后续内容为：
	...
	private String sendData(String type1, String line)
	{
		...
		String i1 = new String(i.toString());
		return i1;
	}
