...
//三亚红树林MIS接口（比Xlhtglsd多一个积分消费），调用动态库（模块名：XLHTGLSD；动态库(dll文件）：KeeperClient.dll；函数：int __stdcall misposTrans(void* input, void* output)；）
public class Xlhtglsd3_PaymentBankFunc extends PaymentBankFunc
{
	String path = getBankPath(paycode);
	String fq = "01";
	public String[] getFuncItem()
	{
		...
		func[11] = "[" + PaymentBank.XKQT5 + "]" + "积分消费撤销";
		func[12] = "[" + PaymentBank.XKQT6 + "]" + "积分消费退货";
		func[13] = "[" + PaymentBank.XKQT7 + "]" + "积分交易查询";
	...
	public boolean getFuncLabel(int type, String[] grpLabelStr)
	{
		switch(type)
		{
			...
			case PaymentBank.XKQT6 : //积分交易退货
				...
				grpLabelStr[4] = "交易金额";
				break;
			case PaymentBank.XKQT7 : //积分交易退货
				grpLabelStr[0] = null;
				grpLabelStr[1] = null;
				grpLabelStr[2] = null;
				grpLabelStr[3] = null;
				grpLabelStr[4] = "积分交易查询";
				break;
		}
...
//			//////////选择：消费、快速消费、分期付款/////////////
			if(type == PaymentBank.XYKXF)
			{
				...
				contents.add(new String[] { "4", "积分消费" });
			...
			if(type == PaymentBank.XYKCX)
			{
				...
				contents.add(new String[] { "2", "积分消费撤销" });
				int choice = new MutiSelectForm().open("请选择消费类型", title, width, contents, true);
				if (choice == -1)
				{
					errmsg = "没有选择消费类型";
...
//			bld.bankinfo = str[8].trim();
			...
			if(type == PaymentBank.XYKXF || type == PaymentBank.XYKCX || type == PaymentBank.XYKTH ||
			   type == PaymentBank.XKQT2 || type == PaymentBank.XKQT3 || type == PaymentBank.XKQT4 ||
