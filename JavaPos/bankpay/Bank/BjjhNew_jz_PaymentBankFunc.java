...
public class BjjhNew_jz_PaymentBankFunc extends PaymentBankFunc
{
	String path = "";
		...
		try
		{
	         ...
	         switch (type)
	         {
	         	...
	         	case PaymentBank.XYKCX:
	         		line = PaymentBank.XYKCX+ "," + syyh + syjh + "," + oldseqno;
	         	break;
...
//        		   bld.trace = Convert.toInt((result[3]).trim());   //流水号
        		   if(result[3].length() >= 166 )
        		   {
        			  bld.cardno =  Convert.newSubString(result[3].trim(), 155, 166); //发卡银行
        		   }
        		   if(result[3].length() >= 207 )
        		   {
        			  bld.cardno =  Convert.newSubString(result[3].trim(), 189, 207); //银行卡号
        		   }
        	   }
