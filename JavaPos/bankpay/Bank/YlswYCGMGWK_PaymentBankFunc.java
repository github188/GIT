...
//宜昌国贸购物卡接口 路径为C:\\gmc\\card
public class YlswYCGMGWK_PaymentBankFunc extends YlswWhf_PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			// 先删除上次交易数据文件
			if (PathFile.fileExist("C:\\gmc\\card\\request.txt"))
			{
				PathFile.deletePath("C:\\gmc\\card\\request.txt");
			...
			if (PathFile.fileExist("C:\\gmc\\card\\result.txt"))
			{
				PathFile.deletePath("C:\\gmc\\card\\result.txt");
			...
			{
				// 调用接口模块
				if (PathFile.fileExist("C:\\gmc\\card\\javaposbank.exe"))
				{
					CommonMethod.waitForExec("C:\\gmc\\card\\javaposbank.exe YLSWWHF");
				}
				else
				{
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			if (!PathFile.fileExist("C:\\gmc\\card\\result.txt") || ((br = CommonMethod.readFileGBK("C:\\gmc\\card\\result.txt")) == null))
			{
			...
			if (Convert.newSubString(result[1], 26, 32).trim().length() > 0)
			{
				...
				try
				{
					bld.trace = Long.parseLong(Convert.newSubString(result[1], 26, 32).trim());
				}
				catch (Exception er)
				{
					er.printStackTrace();
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			try
			{
				pw = CommonMethod.writeFile("C:\\gmc\\card\\request.txt");
				if (pw != null)
				{
					pw.println(line);
					pw.flush();
	...
	public void XYKPrintDoc()
	{
		...
		String printName = "C:\\gmc\\card\\receipt.TXT";
		try
		{
