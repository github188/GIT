...
/**
 * 绍兴中惠,打印签购单时切纸标识符为@ 并且判断在第二次遇到@时不切纸 
 * @author Administrator
 *
 */
public class BjydSxzh_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKReadResult()
	{
		...
		//  
			...
			if(Convert.newSubString(line, 92, 98).trim().equals("")){
				bld.authno= "";
			}else{
				bld.authno = Convert.newSubString(line, 92, 98).trim();
			}
            
			if(Convert.newSubString(line, 104, 110).trim().equals("")){
				bld.memo1= "";
			}else{
				bld.memo1 = Convert.newSubString(line,  104, 110).trim();
			}
