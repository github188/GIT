...
public class Rxtk_PaymentBankFunc extends PaymentBankFunc
{
	public native String creditTransABC(String input,String bankpath);
                              ...
                              String oldauthno, String olddate, Vector memo)
    {
        try
        {
			...
			// 调用接口模块
			if (PathFile.fileExist("c:\\CCB\\MISC.EXE"))
			{
				CommonMethod.waitForExec("c:\\CCB\\MISC.EXE");
			}
			else
			{
				new MessageBox("找不到金卡工程模块 MISC.EXE");
				XYKSetError("XX", "找不到金卡工程模块 MISC.EXE");
				return false;
			}
            ...
            String olddate, Vector memo,StringBuffer arg)
    {
        try
        {
            ...
            String syjh = Convert.increaseChar(ConfigClass.CashRegisterCode,' ', 7);
            ...
            memo1 = Convert.increaseInt(Convert.toInt(bld.crc), 6);
            //根据不同的类型生成文本结构
            switch (type)
            {
    ...
    public boolean XYKReadResult()
    {
        ...
        try
        {
            if (!PathFile.fileExist("C:\\CCB\\response.txt") ||
                    ((br = CommonMethod.readFileGBK("C:\\CCB\\response.txt")) == null))
            {
            ...
            //String CRC = Convert.newSubString(line, 21, 40);
    ...
    public String getErrorInfo(String retcode)
    {
        ...
        try
        {
            ...
            if (!PathFile.fileExist(GlobalVar.ConfigFile+"\\bankError.txt") ||
                    !rtf.loadFile(GlobalVar.ConfigFile+"\\bankError.txt"))
            {
    ...
    public void XYKPrintDoc()
    {
        ...
        try
        {
        	String printName = ConfigClass.BankPath+"\\toprint.txt";
