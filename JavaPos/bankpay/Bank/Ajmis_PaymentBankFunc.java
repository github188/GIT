...
public class Ajmis_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			if (PathFile.fileExist(ConfigClass.BankPath + "\\javaposbank.exe"))
			{
				...
				/**
				 * typedef struct { char TransType[2];
				 * //交易类型（收银机MIS系统传入），消费:"05" 消费撤销:"04" 查询余额"10" char
				 * CardNo[19]; //交易卡号：MISPOS系统返回需记录到数据库中作为对账内容之一 char
				 * Amount[12]; //交易金额（收银机MIS系统传入，需记录到数据库中作为对账内容之一） char Tip[12];
				 * //小费金额（暂时不用，空格补齐） char MisBatchNo[6]; //MIS批次号（暂时不用，空格补齐）
				 * char MisTraceNo[6]; //MIS流水号（暂时不用，空格补齐） char TransTime[6];
				 * //交易时间（MISPOS系统返回） char TransDate[8]; //交易日期（MISPOS系统返回） char
				 * ExpDate[4]; //卡片有效期（MISPOS系统返回） char Track2[37];
				 * //二磁道信息（预留，暂时不用） char Track3[104]; //三磁道信息（预留，暂时不用） char
				 * ReferNo[8]; //系统检索号（MISPOS系统返回，收银机MIS系统在撤销交易时需传给MISPOS系统，
				 * 需记录到数据库中作为对账内容之一） char AuthNo[6]; //MISPOS系统返回 暂时不用，空格补齐 char
				 * ReturnCode[2]; //返回码（MISPOS系统返回，返回码为“00”表示交易成功，否则表示交易失败） char
				 * TerminalId[15]; //MISPOS系统返回交易终端号 char
				 * MerchantId[12];//MISPOS系统返回商户号 char InstallmentTimes[2];
				 * //MISPOS系统返回 暂时不用，空格补齐 char TC[16]; //MISPOS系统返回 暂时不用，空格补齐
				 * char OldAuthDate[8]; //原交易日期，撤销交易时候传送给MISPOS系统 char
				 * MerchantNameEng[50]; //商户名称（英文）MISPOS系统返回 char
				 * MerchantNameChs[40]; //商户中文名称MISPOS系统返回 char
				 * TerminalTraceNo[6]; //终端流水号MISPOS系统返回 char
				 * TerminalBatchNo[6]; //终端批次号MISPOS系统返回 char IcCardId[4];
				 * //MISPOS系统返回，暂时无需处理 char CardType[20]; //MISPOS系统返回 char
				 * TransName[20]; //MISPOS交易中文名称 char
				 * DeviceInitFlag[1];//MISPOS系统返回，暂时无需处理 char
				 * Message[100];//交易失败时，MISPOS系统返回中文错误描述信息 char
				 * Remark[300];//MISPOS系统返回，暂时无需处理 char ForeignCardTraceNo[24];
				 * //MISPOS系统返回，暂时无需处理 char Ttotal[800]; //交易信息汇总，为交易总账信息 char
				 * PlatId[20]; //收银台号 char OperId[20]; //操作员号 } ST_ICBC_MIS;
				 */
				// 交易卡号【19】
				...
				// 返回
				String retmsg = "," + Convert.increaseChar("", 2) + "," + Convert.increaseCharForward(oldauthno, '0', 15) + "," + Convert.increaseChar("", 12) + "," + Convert.increaseChar("", 2) + "," + Convert.increaseChar("", 16) + "," + TransDate + "," + Convert.increaseChar("", 50) + "," + Convert.increaseChar("", 40) + "," + Convert.increaseChar("", 6) + "," + Convert.increaseChar("", 6) + "," + Convert.increaseChar("", 4) + "," + Convert.increaseChar("", 20) + "," + Convert.increaseChar("", 20) + "," + Convert.increaseChar("", 800) + "," + Convert.increaseChar("", 1) + "," + Convert.increaseChar("", 100) + "," + Convert.increaseChar("", 300) + "," + Convert.increaseChar("", 24) + "," + Convert.increaseChar(ConfigClass.CashRegisterCode, 20) + "," + Convert.increaseChar(GlobalInfo.posLogin.gh, 20);
			...
			if (XYKCheckRetCode())
			{
				...
				{
					billcount++;
					if (PathFile.isPathExists(ConfigClass.BankPath + "\\ICBCPRTTKT.txt"))
					{
						String newbillname = "ICBCPRTTKT_" + String.valueOf(billcount) + ".txt";

						PathFile.renameFile(ConfigClass.BankPath + "\\ICBCPRTTKT.txt", ConfigClass.BankPath + "\\" + newbillname);
					}
				}
			}
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			...
			{
				bld.trace = Long.parseLong(ret[21].trim());
				bld.bankinfo = ret[26];
				bld.cardno = ret[1];
			}
			else
	...
	public void XYKPrintDoc()
	{
		...
		try
		{
			if (GlobalInfo.syjDef.printfs == '1' && billcount>0)
				printName = ConfigClass.BankPath + "\\" + "ICBCPRTTKT_" + String.valueOf(billcount) + ".txt";
		
			if (!PathFile.fileExist(printName))
			{
				new MessageBox("未发现[" + "ICBCPRTTKT_" + String.valueOf(billcount)+ "]银联签购单打印文件!");
				return;
			}
