...
import com.efuture.javaPos.Test.MutiSelectForm;

//晋江宝龙，调用动态库（模块名：ICBCKPCLIENT5；动态库(dll文件）：KeeperClient.dll；函数：int __stdcall misposTrans(void* input, void* output)；）
public class IcbcJjBl_PaymentBankFunc extends PaymentBankFunc
{
	String path = "C:\\gmc";
	public String[] getFuncItem()
	{
		...
		func[8] = "[" + PaymentBank.XKQT2 + "]" + "读宝龙瑞特红卡";
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			if(type == PaymentBank.XYKXF)
			{
				...
				contents.add(new String[] { "3", "瑞特红卡消费" });
			...
			if (PathFile.fileExist(path + "\\javaposbank.exe"))
			{
				...
				CommonMethod.waitForExec(path + "\\javaposbank.exe ICBCKPCLIENT5");
			}
			else
			{
				errmsg = "找不到金卡工程模块 ICBCKPCLIENT5";
				XYKSetError("XX", errmsg);
				new MessageBox(errmsg);
...
//		strncpy(inputData.operId,szRequest[6],20);
		try
		{
			...
			line = trans + "," + je + "," + seqno + "," + date + "," + authno + "," + syjh + "," +syyh;
			PrintWriter pw = null;
			try
			{
