...
/**
 * 昆山巴黎春天 农行接口记录银联方返回的折扣金额在YLZK中
 * @author Administrator
 *
 */
public class Ksbl_PaymentBankFunc extends PaymentBankFunc
{
	...
	public String[] getFuncItem()
	{
		...
		func[6] = "[" + PaymentBank.XYKCD + "]" + "重打上笔";
		func[7] = "[" + PaymentBank.XKQT1 + "]" + "重打任意笔";
		func[8] = "[" + PaymentBank.XKQT2 + "]" + "业务菜单";
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			if ((type != PaymentBank.XYKXF) && (type != PaymentBank.XYKCX) && (type != PaymentBank.XYKTH) && (type != PaymentBank.XYKQD)
					&& (type != PaymentBank.XYKYE) && (type != PaymentBank.XYKJZ) && (type != PaymentBank.XYKCD) && (type != PaymentBank.XKQT1)
	...
	public boolean XYKNeedPrintDoc()
	{
		...
		if ((type == PaymentBank.XYKXF) || (type == PaymentBank.XYKCX) || (type == PaymentBank.XYKTH) || (type == PaymentBank.XYKYE)
				|| (type == PaymentBank.XYKJZ) || (type == PaymentBank.XYKCD) || (type == PaymentBank.XKQT1) || (type == PaymentBank.XKQT2))
		{
			return true;
		}
		else
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			line = type1 + jestr + extrainfo + syyh + syjh;
			PosLog.getLog(getClass()).debug("request.txt"+line);
			try
			{
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			...
			PosLog.getLog(getClass()).debug("result.txt"+line);
			...
			if (result.length >= 2)
			{
				...
				if (temp.length > 20 && !temp[20].trim().equals(""))
				{
					bld.ylzk = ManipulatePrecision.div(Convert.toDouble(temp[20]), 100.00);
				}
			}
