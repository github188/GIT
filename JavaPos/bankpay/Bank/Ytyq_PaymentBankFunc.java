...
//永泰用券消费
public class Ytyq_PaymentBankFunc extends PaymentBankFunc {
	...
	public String[] getFuncItem()
	{
		...
		func[0] = "[" + PaymentBank.XYKXF + "]" + "券消费";
		func[1] = "[" + PaymentBank.XYKCX + "]" + "券撤销";
		func[2] = "[" + PaymentBank.XYKTH + "]" + "券退货";
		func[3] = "[" + PaymentBank.XYKCD + "]" + "券退货撤销";
		return func;
	}
	...
	public boolean checkBankOperType(int operType, SaleBS saleBS, PaymentBank payObj)
	{
		...
		if (saleBS != null)
		{
			...
			((SellType.ISSALE(saleBS.saletype) || saleBS.isRefundStatus()) && operType != PaymentBank.XYKXF&& operType != PaymentBank.XYKCX)
					|| ((SellType.ISBACK(saleBS.saletype) && !saleBS.isRefundStatus()) && operType != PaymentBank.XYKCD && operType != PaymentBank.XYKTH))
			{
				ok = false;
			}
		}
		else
		{
			...
			(payObj != null && operType != PaymentBank.XYKCX && operType != PaymentBank.XYKCD)
					|| (payObj != null && SellType.ISHC(payObj.salehead.djlb) && operType != PaymentBank.XYKCX&& operType != PaymentBank.XYKCD)
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
	      ...
	      if(track2!=null){
			...
			PosLog.getLog(this.getClass().getSimpleName()).info("request=[" + request[1] + "]");
			
			// 调用接口模块
			if (RdPlugins.getDefault().getPlugins1().exec(Convert.toInt(request[0]), request[1])) {
			    bonusAlterReturn = (String) RdPlugins.getDefault()
						.getPlugins1().getObject();
			}
			// 读取应答数据
			if (!XYKReadResult(bonusAlterReturn)) { return false; }
...
//		String fphm = String.valueOf(saleBS.saleHead.fphm);
		String fphm="0";
	...
	public boolean XYKReadResult(String result)
	{
		...
		try
		{
			PosLog.getLog(this.getClass().getSimpleName()).info("result=[" + result + "]");
			...
			try
			{
				...
				switch (type)
				{
					case PaymentBank.XYKXF: // 泰豆消费
						 ...
						 **/
						type1="3";
						bonusAlterRequest= mktcode + "," + syyh + ","+ track2 + "," + fphm + ",1," + crashNum + ","+ money + ",0,2";
						break;
					case PaymentBank.XYKCX: // 泰豆撤销
						...
						+ "," + fphm + "," + oldseqno + "," 
						+ oldauthno  + ","+ olddate + "," + Convert.toInt(money);;
						break;
					case PaymentBank.XYKTH: // 隔日退货
						 ...
						 */
						type1 = "5";
						bonusAlterRequest =mktcode+ "," + syyh+ ","+ track2 + "," + fphm
						+ "," + oldseqno + "," + oldauthno  + "," + olddate + "," + yfphm
						+ ",1," + crashNum +","+ money + ",0";
						break;
					case PaymentBank.XYKCD: // 退货撤销
