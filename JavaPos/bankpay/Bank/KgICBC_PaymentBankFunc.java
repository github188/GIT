...
public class KgICBC_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			if (saleBS != null)
			{
				syyh = Convert.increaseChar(saleBS.saleHead.syyh, ' ', 8);
				syjh = Convert.increaseChar(saleBS.saleHead.syjh, ' ', 8);
				fphm = Convert.increaseChar(String.valueOf(saleBS.saleHead.fphm), ' ', 16);
				line = type1 + jestr + syyh + syjh + fphm;
			}
			else
			{
				syyh = Convert.increaseChar(GlobalInfo.posLogin.gh, ' ', 8);
				syjh = Convert.increaseChar(GlobalInfo.syjDef.syjh, ' ', 8);
				fphm = Convert.increaseChar(String.valueOf(GlobalInfo.syjStatus.fphm), ' ', 16);
				line = type1 + jestr + syyh + syjh + fphm;
			}
			try
			{
		...
		try
		{
			...
			if (result.length > 0 && result[0].trim().equals("1"))
			{
				if (result.length > 1)
				{
					...
					// 如果返回值为非0，或者交易类型为其他交易类型时返回False
					if (!bld.retcode.equals("00"))
						return false;

					if ((Integer.parseInt(bld.type.trim()) == PaymentBank.XKQT1))
						return true;
					...
					bld.cardno = Convert.newSubString(result[1].trim(), 14, 33).trim();
					bld.authno = Convert.newSubString(result[1].trim(), 33, 45).trim();
					bld.trace = Long.parseLong(Convert.newSubString(result[1].trim(), 49, 55).trim());
					bld.bankinfo = Convert.newSubString(result[1].trim(), 99, 119).trim();
					bld.retmsg = Convert.newSubString(result[1].trim(), 119, 151).trim();
				}
			}
			else
			{
	...
	public void XYKPrintDoc()
	{
		...
		try
		{
			...
			if (!PathFile.fileExist("C:\\JavaPos\\toprint.txt"))
			{
				...
				new MessageBox("未找到签购单文件无法打印!", null, false);
				return;
			}
			else
			{
