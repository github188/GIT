...
//大金新百安庆店银联接口，调用动态库（模块名：AQXB；动态库(dll文件）：KeeperClient.dll；函数：int __stdcall misposTrans(void* input, void* output)；）
public class Aqxb_PaymentBankFunc extends PaymentBankFunc
{
	String path = "C:\\gmc";
	public String[] getFuncItem()
	{
		...
		func[9] = "[" + PaymentBank.XKQT4 + "]" + "积分消费退货";
		func[10] = "[" + PaymentBank.XKQT3 + "]" + "快速消费交易";
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			if(type == PaymentBank.XYKXF)
			{
				...
				contents.add(new String[] { "3", "积分消费" });
...
//				new MessageBox("javaposbank.exe文件路径："+path);
				CommonMethod.waitForExec(path + "\\javaposbank.exe AQXB");
			}
			else
			{
				errmsg = "找不到金卡工程模块 AQXB";
				XYKSetError("XX", errmsg);
				new MessageBox(errmsg);
...
//		strncpy(inputData.AddDatas,szRequest[8],256);
		...
		try
		{
			...
			line = trans + "," + je + "," + seqno + "," + date + "," + authno + "," + syjh + "," +syyh + "," + input1 + "," + input2;       
			PrintWriter pw = null;
			try
			{
