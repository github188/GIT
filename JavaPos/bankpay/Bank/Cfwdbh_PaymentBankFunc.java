...
//赤峰万达百货银联接口
//调用动态库（模块名：CFWDBH；动态库(dll文件）：softpos0.dll；函数：int CreditTransABC( char *strin, char *strout )；）
public class Cfwdbh_PaymentBankFunc extends PaymentBankFunc{
	    String path = null;
...
//				获得金卡文件路径
	            ...
	            {
	            	CommonMethod.waitForExec(path+"\\javaposbank.exe CFWDBH","javaposbank");
	            }
	            else
	            {
			...
			try
	        {
	            ...
	            if (line.length() >= 2)
	            {
	            	...
	            	if(type == PaymentBank.XYKXF || type == PaymentBank.XYKCX ||  type == PaymentBank.XYKTH )
	            	{
	                	...
	                	bld.trace = Integer.parseInt(line.substring(34,40));   //流水号6
