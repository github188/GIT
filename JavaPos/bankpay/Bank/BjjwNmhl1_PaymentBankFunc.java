...
//洛阳万达
public class BjjwNmhl1_PaymentBankFunc extends PaymentBankFunc {
	...
	public boolean XYKExecute(int type, double money, String track1,
		...
		try {
			...
			if (PathFile.fileExist(ConfigClass.BankPath + "\\javaposbank.exe")) {
				if (PathFile.fileExist(ConfigClass.BankPath
						+ "\\ICBCPRTTKT.txt")) {
					PathFile.deletePath(ConfigClass.BankPath
							+ "\\ICBCPRTTKT.txt");
				...
				if (PathFile.fileExist(ConfigClass.BankPath + "\\request.txt")) {
					PathFile.copyPath(ConfigClass.BankPath + "\\request.txt",
							ConfigClass.BankPath + "\\LastRequest.txt");
				...
				if (PathFile.fileExist(ConfigClass.BankPath + "\\result.txt")) {
					PathFile.copyPath(ConfigClass.BankPath + "\\result.txt",
							ConfigClass.BankPath + "\\LastResult.txt");
				 ...
				 * ReferNo[8];
				 * //系统检索号（MISPOS系统返回，收银机MIS系统在撤销交易时需传给MISPOS系统，需记录到数据库中作为对账内容之一）
				 * char AuthNo[6]; //MISPOS系统返回 暂时不用，空格补齐 char ReturnCode[2];
				 * //返回码（MISPOS系统返回，返回码为“00”表示交易成功，否则表示交易失败） char
				 * TerminalId[15]; //MISPOS系统返回交易终端号 char MerchantId[12];
				 * //MISPOS系统返回商户号 char InstallmentTimes[2]; //MISPOS系统返回
				 * 暂时不用，空格补齐 char TC[16]; //MISPOS系统返回 暂时不用，空格补齐 char
				 * OldAuthDate[8]; //原交易日期，撤销交易时候传送给MISPOS系统 char
				 ...
				 * TransName[20]; //MISPOS交易中文名称 char DeviceInitFlag[1];
				 * //MISPOS系统返回，暂时无需处理 char Message[100];
				 * //交易失败时，MISPOS系统返回中文错误描述信息 char Remark[300];
				 * //MISPOS系统返回，暂时无需处理 char ForeignCardTraceNo[24];
				...
				// 系统检索号【8】
				String ReferNo = Convert.increaseCharForward(oldseqno,'0', 8);
				// MISPOS系统返回【6】
				...
				String retmsg = ","
						...
						+ ","
						+ Convert
								.increaseChar(ConfigClass.CashRegisterCode, 20)
						+ ","
						+ Convert.increaseChar(GlobalInfo.posLogin.gh, 20);

				line = type1 + "," + cardno + "," + jestr + "," + Tip + ","
						+ MisBatchNo + "," + MisTraceNo + "," + TransTime + ","
						+ Convert.increaseChar("", 8) + "," + ExpDate + ","
						+ Track2 + "," + Track3 + "," + ReferNo + "," + AuthNo
						+ retmsg;

				PrintWriter pw = null;

				try {
					pw = CommonMethod.writeFile(ConfigClass.BankPath
							+ "\\request.txt");
				...
				CommonMethod.waitForExec(ConfigClass.BankPath
						+ "\\javaposbank.exe BJJWNMHL", "javaposbank.exe");
			} else {
	...
	public boolean XYKReadResult() {
		...
		try {
			if (!PathFile.fileExist(ConfigClass.BankPath + "\\result.txt")
					|| ((br = CommonMethod.readFileGBK(ConfigClass.BankPath
							+ "\\result.txt")) == null)) {
	...
	protected boolean XYKNeedPrintDoc(int type) {
		if (type != PaymentBank.XYKXF && type != PaymentBank.XYKTH
				&& type != PaymentBank.XYKCX && type != PaymentBank.XYKCD
