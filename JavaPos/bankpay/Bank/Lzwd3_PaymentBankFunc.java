...
public class Lzwd3_PaymentBankFunc extends PaymentBankFunc
{
	String path = null;
...
//        	char merchant_no[15];	// 49～63位   商户号15
//        	char terminal_no[8];	// 64～71位   终端号8
//        	char data[8];			// 72～79位  交易日期8
//        	char time[6];			// 80～85位  交易时间6
//        	char batch_num[6];		// 86～91位  批次号6
//        	char reference_num[12];//  92～103位  参考号12
//        	char auth_id[6];		// 104～109 授权码6
//        	char resp_chin[40];		/* 49～88位（  110～149位）  错误说明(40字节，左对齐，不足部分补空格)*/
//          char lrc[3];		/*89～91位 （150～152）空格	交易数据校验码*/
            ...
            if (result.length >= 2)
            {
            	...
            	{ 
//            		bld.retmsg = Convert.newSubString(result[1],48,88);//错误说明
            		bld.retmsg = Convert.newSubString(result[1],109,149);//错误说明
            		return false;
            	}
...
//            	String lrc = Convert.newSubString(result[1],88,91);
            	String lrc = Convert.newSubString(result[1],149,152);//交易数据校验码
