...
*北京燕莎奥莱第二版
*/
public class YsAlMzk1_PaymentBankFunc extends PaymentBankFunc
{
	public String[] getFuncItem()
	{
		...
		func[7] = "[" + PaymentBank.XYKCD+ "]" + "重打上一笔";
		return func;
	}
...
//			if (PathFile.fileExist("C:\\JavaPos\\dat\\print.txt"))
//			{
//				PathFile.deletePath("C:\\JavaPos\\dat\\print.txt");
//
//				if (PathFile.fileExist("C:\\JavaPos\\dat\\print.txt "))
//				{
...
//			}
			...
			if (PathFile.fileExist("C:\\JavaPos\\dat\\settle_t.txt"))
			{
				PathFile.deletePath("C:\\JavaPos\\dat\\settle_t.txt");

				if (PathFile.fileExist("C:\\JavaPos\\dat\\settle_t.txt"))
				{
					errmsg = "交易请求文件settle_t.txt无法删除,请重试";
					XYKSetError("XX", errmsg);
					new MessageBox(errmsg);
					return false;
				}
			...
			if (!XYKWriteRequest(type, money, track1, track2, track3, oldseqno, oldauthno, olddate, memo)) { return false; }
				...
				if (XYKNeedPrintDoc())
				{
					if (type != PaymentBank.XYKQD && type != PaymentBank.XYKYE && type !=PaymentBank.XKQT2)
					{
						XYKPrintDoc(type);
					}
				}
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			String strtypecode = "";//交易类型
		
			String strje = "";//金额
			strje = Convert.increaseCharForward(String.valueOf((long) ManipulatePrecision.doubleConvert(money * 100, 2, 1)), ' ', 12);
			
			String strseqno = "";//原流水号
			if (oldseqno != null)
			{
			...
			{
				strseqno = Convert.increaseChar("0",12);
			}
			
			String stroldauthno = "";//参考号
			if (oldauthno != null)
			{
			...
			{
				oldauthno = Convert.increaseChar("0",12);
			}
			...
			{
				strolddate = Convert.increaseChar("0", 8);
			}

			String strtrack2 = track2;
			//String strtrack3 = "";
			...
			// 传入串
			//String line = strtypecode +"|"+ strje +"|" + stroldauthno +"|"  + strseqno +"|" + strolddate +"|" + strtrack2 +"|"+ strtrack3+"|";
			String line = strtypecode +"|"+ strje +"|" + stroldauthno +"|"  + strseqno +"|" + strolddate +"|"+"|"+GlobalInfo.syjStatus.syyh+"|"  + strtrack2 +"|";
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			...
			bld.memo = ret[0].split(",")[0];
			
			if (bld.memo.equals("0"))
			{
				bld.retcode = ret[6].trim();
				bld.retmsg = ret[7].trim();
				bld.cardno = ret[4].trim();
				bld.trace = Long.parseLong(ret[2].trim());
				bld.authno = ret[3].trim();
				bld.je = ManipulatePrecision.doubleConvert(Double.parseDouble(ret[1].trim()), 2, 1);
				errmsg = bld.retmsg;
			}
			else
			{
	...
	public void XYKPrintDoc(int type)
	{
		...
		try
		{
			...
			{
				printName ="C:\\JavaPos\\dat\\settle_t.txt";
			}
			else
			{
				printName ="C:\\JavaPos\\dat\\print.txt";
			}
