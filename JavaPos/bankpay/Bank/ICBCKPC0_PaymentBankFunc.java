...
public class ICBCKPC0_PaymentBankFunc  extends PaymentBankFunc
{
	...
	public boolean getFuncText(int type, String[] grpTextStr)
	{
		...
		case PaymentBank.XYKJZ: // 内卡结账
			...
			grpTextStr[2] = null;
			grpTextStr[3] = null;
			grpTextStr[4] = "按回车键弿ƾ结账";

			break;

		case PaymentBank.XYKCD: // 签购单重打上笿
			grpTextStr[0] = null;
			grpTextStr[1] = null;
			grpTextStr[2] = null;
			grpTextStr[3] = null;
			grpTextStr[4] = "重打上笔签购单";

			break;

		case PaymentBank.XKQT1: // 签购单重打指宿
			grpTextStr[0] = null;
			grpTextStr[1] = null;
			grpTextStr[2] = null;
			grpTextStr[3] = null;
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			if (PathFile.fileExist(ConfigClass.BankPath + "\\javaposbank.exe"))
			{
				 ...
				 * typedef struct
				 * {
				 * char TransType[2]; //交易类型（收银机MIS系统传入），消费:"05" 消费撤销:"04"
				 * 查询余额"10"
				 * char CardNo[19]; //交易卡号：MISPOS系统返回霿ΰ录到数据库中作为对账内容之一
				 * char Amount[12]; //交易金额（收银机MIS系统传入，需记录到数据库中作为对账内容之丿܉
				 * char Tip[12]; //小费金额（暂时不用，空格补齐＿
				 * char MisBatchNo[6]; //MIS批次号（暂时不用，空格补齐）
				 * char MisTraceNo[6]; //MIS流水号（暂时不用，空格补齐）
				 * char TransTime[6]; //交易时间（MISPOS系统返回＿
				 * char TransDate[8]; //交易日期（MISPOS系统返回＿
				 * char ExpDate[4]; //卡片有效期（MISPOS系统返回＿
				 * char Track2[37]; //二磍道信息（预留，暂时不用）
				 * char Track3[104]; //三磍道信息（预留，暂时不用）
				 * char ReferNo[8];
				 * //系统棿Ԣ号（MISPOS系统返回，收银机MIS系统在撤锿ڤ易时霿ܠ给MISPOS系统，
				 * 需记录到数据库中作为对账内容之丿܉
				 * char AuthNo[6]; //MISPOS系统返回 暂时不用，空格补齿
				 * char ReturnCode[2]; //返回码（MISPOS系统返回，返回码为⿰0”表示交易成功，否则表示交易失败＿
				 * char TerminalId[15]; //MISPOS系统返回交易终端叿
				 * char MerchantId[12]; //MISPOS系统返回商户叿
				 * char InstallmentTimes[2]; //MISPOS系统返回 暂时不用，空格补齿
				 * char TC[16]; //MISPOS系统返回 暂时不用，空格补齿
				 * char OldAuthDate[8]; //原交易日期，撤销交易时怤ܠꀧۙMISPOS系统
				 ...
				 * char IcCardId[4]; //MISPOS系统返回，暂时无霿Đ琿
				 * char CardType[20]; //MISPOS系统返回
				 * char TransName[20]; //MISPOS交易中文名称
				 * char DeviceInitFlag[1]; //MISPOS系统返回，暂时无霿Đ琿
				 * char Message[100]; //交易失败时，MISPOS系统返回中文错误描述信息
				 * char Remark[300]; //MISPOS系统返回，暂时无霿Đ琿
				 * char ForeignCardTraceNo[24]; //MISPOS系统返回，暂时无霿Đ琿
				 * char Ttotal[800]; //交易信息汇瀯܌为交易总账信息
				 * char PlatId[20]; //收银台号
				 * char OperId[20]; //操作员号
				 * } ST_ICBC_MIS;
				 */
				...
				// 返回
				String retmsg = "," + Convert.increaseChar("", 2) + "," + Convert.increaseChar(oldauthno, ' ', 15) + "," + Convert.increaseChar("", 12) + "," + Convert.increaseChar("", 15) + "," + Convert.increaseChar("", 2) + "," + Convert.increaseChar("", 16) + "," + TransDate + "," + Convert.increaseChar("", 50) + "," + Convert.increaseChar("", 40) + "," + Convert.increaseChar("", 6) + "," + Convert.increaseChar("", 6) + "," + Convert.increaseChar("", 4) + "," + Convert.increaseChar("", 20) + "," + Convert.increaseChar("", 20) + "," + Convert.increaseChar("", 800) + "," + Convert.increaseChar("", 1) + "," + Convert.increaseChar("", 100) + "," + Convert.increaseChar("", 300) + "," + Convert.increaseChar("", 24) + "," + Convert.increaseChar(ConfigClass.CashRegisterCode, 20) + "," + Convert.increaseChar(GlobalInfo.posLogin.gh, 20);
				bld.type = type1;
			...
			{
				new MessageBox("找不到金卡工程模块javaposbank.exe");
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			...
			{
				if (bld.type.equals("14") || bld.type.equals("09") || bld.type.equals("10"))
					return true;
				if (ret.length > 25)
					bld.bankinfo = ret[25];
				if (ret.length > 1)
					bld.cardno = ret[1];
				if (ret.length > 11)
					bld.trace = Long.parseLong((ret[11]));
			}
			else
			{
				if (ret.length > 28)
					bld.retmsg = ret[29].trim();
			}
