...
public class Shsd_PaymentBankFunc extends PaymentBankFunc
{
	class ResponseData
	{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			if (type != PaymentBank.XYKQD && type != PaymentBank.XYKJZ && type != PaymentBank.XYKXF && type != PaymentBank.XYKCX && type != PaymentBank.XYKTH)
			{
				errmsg = "银联接口不支持该交易";
			...
			if (type != PaymentBank.XYKQD && type != PaymentBank.XYKJZ)
			{
				if (!(memo != null && memo.size() > 0))
				{
					...
					contents.add(new String[] { "00", "杉德pos自动识别卡" });
					contents.add(new String[] { "01", "银行卡" });
					contents.add(new String[] { "03", "斯玛特卡" });
	...
	public boolean XYKReadResult()
	{
		try
		{
			...
			response.OperateType = linelist.length >=1?linelist[0].trim():"";
			response.TransType = linelist.length >=2?linelist[1].trim():"";
			response.CardType = linelist.length >=3?linelist[2].trim():"";
			response.ResponseCode = linelist.length >=4?linelist[3].trim():"";
			response.ResponseMsg = linelist.length >=5?linelist[4].trim():"";
			response.CashRegNo = linelist.length >=6?linelist[5].trim():"";
			response.CasherNo = linelist.length >=7?linelist[6].trim():"";
			response.Amount = linelist.length >=8?linelist[7].trim():"";
			response.SettleNum = linelist.length >=9?linelist[8].trim():"";
			response.MerchantID = linelist.length >=10?linelist[9].trim():"";
			response.MerchantName = linelist.length >=11?linelist[10].trim():"";
			response.TerminalID = linelist.length >=12?linelist[11].trim():"";
			response.CardNo = linelist.length >=13?linelist[12].trim():"";
			response.Exp_Date = linelist.length >=14?linelist[13].trim():"";
			response.BankNo = linelist.length >=15?linelist[14].trim():"";
			response.TransDate = linelist.length >=16?linelist[15].trim():"";
			response.TransTime = linelist.length >=17?linelist[16].trim():"";
			response.Auth_Code = linelist.length >=18?linelist[17].trim():"";
			response.SysRefNo = linelist.length >=19?linelist[18].trim():"";
			response.CashTraceNo = linelist.length >=20?linelist[19].trim():"";
			response.OriginTrace = linelist.length >=21?linelist[20].trim():"";
			response.SysTraceNo = linelist.length >=22?linelist[21].trim():"";
			response.OriginSysTrace = linelist.length >=23?linelist[22].trim():"";
			response.MemoItem = linelist.length >=24?linelist[23].trim():"";
			...
			if (response.CashTraceNo.length() > 0)
			{
