...
//保定隆基泰和联名卡接口
//新写了一个调用 int func_name(char *in, char *ou) 模式的模块，模块名 STRING
//写入request.txt文件参数样式  dll文件名,函数名,传入参数
public class RkysBd_PaymentBankFunc extends PaymentBankFunc
{
	String path;
	...
	public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
	{
		try
		{
            ...
            if (bld.retbz != 'Y')
            {
                ...
                {
                	CommonMethod.waitForExec(path + "\\javaposbank.exe STRING");
                }
                else
                {
	...
	public boolean XYKWriteRequest(int type, double money, String track1,String track2, String track3,String oldseqno, String oldauthno,String olddate, Vector memo)
	{
		 ...
		 try
		 {
	         ...
	         String t2 = Convert.increaseChar(track2,' ',37);
	         String t3 = Convert.increaseChar(track3,' ',104);
	         String pass = Convert.increaseChar("  ",' ',8);
	         
	         //dll文件名，函数名，传入参数
//	         line = "trans_ricom.dll," + "MenuApp_Ricom," + type1 + jestr + syyh + syjh + fphm + cardno + t2 + t3 + pass;
	         line = "trans.dll," + "MenuApp," + type1 + jestr + syyh + syjh + fphm + cardno + t2 + t3 + pass;
            ...
            if (result.length > 0 && result[0].trim().equals("1"))
            {
	            if (result.length > 1)
	            {
		            ...
		            if (result.length >= 2 && (type != PaymentBank.XKQT1))
		            {
		            	...
		            	bld.cardno 		= Convert.newSubString(result[1].trim(), 26, 45);
