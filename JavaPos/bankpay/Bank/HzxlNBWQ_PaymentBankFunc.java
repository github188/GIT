 ...
 *
 */
public class HzxlNBWQ_PaymentBankFunc extends PaymentBankFunc
{
	String path = null;
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			if (bld.retbz != 'Y')
			{
				...
				{
					CommonMethod.waitForExec(path + "\\javaposbank.exe HZXL", "javaposbank.exe");
				}
				else
				{
	...
	protected boolean XYKNeedPrintDoc()
	{
        ...
        if ((type == PaymentBank.XYKXF) || (type == PaymentBank.XYKCX) ||
                (type == PaymentBank.XYKTH) || (type == PaymentBank.XYKCD) ||   /*(type == PaymentBank.XYKJZ) */
...
//					}
	...
	public void XYKPrintDoc()
	{
		...
		try
		{
			if (!PathFile.fileExist(path + "\\bank\\receipt.txt"))
			{
...
//			for (int i = 0; i < GlobalInfo.sysPara.bankprint; i++)
			for (int i = 0; i < 1; i++)
			{
				...
				try
				{
					br = CommonMethod.readFileGBK(path + "\\bank\\receipt.txt");
