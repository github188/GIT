...
public class Ylgm1_PaymentBankFunc extends PaymentBankFunc{
	    String path = null;
	    String fq = null;
...
//	              调用接口模块
	            ...
	            {
	            	CommonMethod.waitForExec(path+"\\javaposbank.exe YLGM");
	            }
	            else
	            {
...
//				    char lrc[3];       /*223～225位空格	交易数据校验码（3字节）*/
		         ...
		         //收银机号+操作员号+交易类型+金额+ 小费金额 +原交易日期+原交易参考号 +授权码+原交易终端号+分期付款期数+二磁道数据+三磁道数据 + 校验码
		         line = syjh + syyh + type1 + jestr + TipsAmount + date + seqno + authno + TerNo +fqno+ strack2 + strack3 + bld.crc;
...
//		char lrc[3];/*165～167位空格	交易数据校验码（3字节）*/
		...
		public boolean XYKReadResult()
		{
			...
			try
	        {
	            ...
	            if (result.length >= 2)
	            {
	            	...
	            	{
	            		bld.retmsg = Convert.newSubString(result[1], 64, 164);    //错误信息
	            		return false;
	            	}
	            	   ...
	            	   type == PaymentBank.XKQT1 || type == PaymentBank.XKQT2 )
	            	{
	                	...
	                	bld.trace = Integer.parseInt(Convert.newSubString(result[1], 46, 52));   //流水号6
	                	bld.je = ManipulatePrecision.doubleConvert(ManipulatePrecision.div(Convert.toDouble(Convert.newSubString(result[1], 52, 64)),100),2,1); //金额12 
	            	}  
	            	
	            	String lrc = Convert.newSubString(result[1], 164, 167);   //交易数据校验码
