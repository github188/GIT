 ...
 * 新疆汇嘉门店切换
 */
public class SnYl_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
	{
		try
		{
            ...
            if (bld.retbz != 'Y')
            {
                ...
                {
                	CommonMethod.waitForExec(ConfigClass.BankPath+"\\javaposbank.exe SNYL");
                }
                else
                {
	 ...
	 public void XYKPrintDoc_Print(String printStr)
		{
			if (onceprint)
			{
				...
				if (port == null || port.length() <= 0) Printer.getDefault().printLine_Normal(printStr);
		...
		public void XYKPrintDoc_End()
		{
			...
			{
				String port = getBankClassConfig("PRINTPORT");
				if (port == null || port.length() <= 0) Printer.getDefault().cutPaper_Normal();
				else if (port.trim().equals("1"))
				{
