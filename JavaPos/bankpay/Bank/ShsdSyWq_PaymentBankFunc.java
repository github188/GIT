...
//沈阳北一路万达百货  客户需javapos调用打印机打印签购单
public class ShsdSyWq_PaymentBankFunc extends PaymentBankFunc
{
	...
	protected String getBankClassConfig(String attr)
	{
		...
		if (attr.equals("REQCHECKDATETIME")) { return "YYYYMMDD"; }
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector cardtype)
	{
		...
		try
		{
			...
			sbstr.append(Convert.increaseCharForward(GlobalInfo.syjDef.syjh, '0', 6) + ",");
			sbstr.append(Convert.increaseCharForward(GlobalInfo.posLogin.gh, '0', 6) + ",");
			...
			if (type == PaymentBank.XYKCD){
				sbstr.append(Convert.increaseChar("B0000625",8));
			}
	...
	public void XYKPrintDoc()
	{
		...
		try
		{
			...
			if (type == PaymentBank.XYKXF || type == PaymentBank.XYKCX || type == PaymentBank.XYKTH || type == PaymentBank.XYKJZ|| type == PaymentBank.XYKCD)
			{
				if (!PathFile.fileExist("c:\\libSand\\print\\sand_print.txt"))
				{
				...
				{
					printName = "c:\\libSand\\print\\sand_print.txt";
				}
			}
			else
			{
		...
		finally
		{
			...
			if (PathFile.fileExist("c:\\libSand\\print\\sand_print.txt"))
            {
                PathFile.deletePath("c:\\libSand\\print\\sand_print.txt");
            }
		}
	}
}
