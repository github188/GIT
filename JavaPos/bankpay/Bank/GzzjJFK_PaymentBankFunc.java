...
public class GzzjJFK_PaymentBankFunc extends PaymentBankFunc
{
...
//	    private String resp_code_out = "";		  //交易响应码
	    ...
	    private String pri_jifen_out ="";         //私有积分
	    private String pub_jifen_out ="";          //公共积分
	    private String card_valid_out= "";        //卡有效期
    ...
    public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
    {
        try
        {
            ...
            if ((type != PaymentBank.XYKXF) && (type != PaymentBank.XYKCX) && (type != PaymentBank.XYKQD)&& (type != PaymentBank.XYKYE))
            {
                errmsg = "银联接口不支持该交易";
                                      ...
                                      String olddate, Vector memo)
    {
        try
        {
        	...
        	{
        		line = 3+"," + zzh.trim()+","+pos_mode.trim()+","+track2.trim()+","+track3.trim()+","+mid.trim()+","+addition_data.trim()+","+pin.trim()+","+bat_no.trim();
        		//line = 2+","+zzh+","+jestr+","+pos_mode+","+order_no+","+track2+","+track3+","+mid+","+pin+","+original_trace_no+","+bat_no;
        	}
...
//			    resp_code_out = lines[8].trim();
			    ...
			    trans_time_out = lines[3].trim();
			    trans_date_out = lines[4].trim();
			    card_valid_out = lines[5].trim();
			    sys_no_out = lines[6].trim();
			    term_no_out = lines[8].trim();
			    addition_account = lines[9].trim();
			    
			    pri_jifen_out = lines[10].trim();
			    pub_jifen_out = lines[11].trim();
			}
    ...
    {
    	ProgressBox pb = null;
        //   new MessageBox("99999999999999");
           try
           {	//new MessageBox("000000000000000");
               int type = Integer.parseInt(bld.type.trim());
             //  new MessageBox("aaaaaaaaaaaa");
               Object obj  = GlobalInfo.localDB.selectOneData("select PRIV from SYJMAIN where SYJH='" +GlobalInfo.syjStatus.syjh + "'" );
   			String name = "";//商户名称
               if (obj != null && !String.valueOf(obj).equals(""))
   			{
               //	new MessageBox("bbbbbbbbbbbb");
   				name = String.valueOf(obj).trim();
   			}
             //  new MessageBox("cccccccccccccccc");
               StringBuffer sb = new StringBuffer();
           	for (int i = 0;i < 3;i++)
           	{        		
   	        	sb.append("		 正佳积分卡 	" + "\n");
   	        	sb.append("商户名称:	"+name + "\n");
   	        	sb.append("商户编号:	"+GlobalInfo.syjStatus.syjh + "\n");
   	        	...
   	        	{
   	        		sb.append("私有积分： "+pri_jifen_out + "\n");
   	        		sb.append("公共积分： "+pub_jifen_out + "\n");
   	        		sb.append("卡有效期： "+card_valid_out + "\n");
   	        	}
           	...
           	//new MessageBox("ddddddddddddddd");
           	...
           	//new MessageBox("eeeeeeeeeeeeeee");
               pb = new ProgressBox();
