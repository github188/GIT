...
/**
*国光百货银联接口：修改中亚糖酒(无界面模式)
*/
public class Ggbh_PaymentBankFunc extends PaymentBankFunc
{	
...
//			 返回码
			...
			String LRC = newLine.substring(newLine.length()-3, newLine.length());
			...
			if(type == PaymentBank.XYKQD ||  type == PaymentBank.XYKJZ || type == PaymentBank.XYKYE || type == PaymentBank.XYKCD)
			{
				return true;
			}
			// 卡号
			...
			// 流水号
			bld.trace = Integer.parseInt(newLine.substring(26,32)); 
			// 金额
			...
			bld.je = j;
			// 系统参考号
			bld.authno = newLine.substring(newLine.length()-25, newLine.length()-13);
			// LRC
