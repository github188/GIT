...
public class RkysSjzWQ_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
	{
		try
		{
			new MessageBox("XYKExecute Start");
            ...
            if (bld.retbz != 'Y')
            {
                ...
                {
                	CommonMethod.waitForExec("C:\\JavaPos\\javaposbank.exe 红府", "javaposbank.exe");
                }
                else
                {
	...
	{
		new MessageBox("XYKNeedPrintDoc Start");
		if (!checkBankSucceed())
        {
            return false;
        }
		new MessageBox("XYKNeedPrintDoc end");
	...
	{
		if (!bld.type.equals(String.valueOf(PaymentBank.XKQT1)) && bld.retbz == 'N')
		{
			errmsg = bld.retmsg;
	...
	public boolean XYKReadResult()
	{
		...
		try
        {
            ...
            if (result.length >= 2)
            {
    			...
    			bld.retmsg = Convert.newSubString(result[1], 119, 151).trim();
                int type = Integer.parseInt(bld.type.trim());
    			if ("00".equals(bld.retcode) && type != PaymentBank.XKQT1)
    			{
    				bld.cardno = Convert.newSubString(result[1], 14, 33).trim();
    				String je = Convert.newSubString(result[1], 2, 14);
    				double j = Double.parseDouble(je);
    				j = ManipulatePrecision.mul(j, 0.01);
    				bld.je = j;
    				bld.trace = Long.parseLong(Convert.newSubString(result[1], 33, 45).trim());
    				bld.bankinfo = Convert.newSubString(result[1], 99, 119);
    			}
            }
