...
public class Ylgm_PaymentBankFunc extends PaymentBankFunc{
	    String path = null;
	    String fq = null;
...
//	              调用接口模块
	            ...
	            {
	            	CommonMethod.waitForExec(path+"\\javaposbank.exe YLGM","javaposbank");
	            }
	            else
	            {
			...
			try
	        {
	            ...
	            if (result.length >= 2)
	            {
	            	   ...
	            	   type == PaymentBank.XKQT1 || type == PaymentBank.XKQT2 )
	            	{
	                	...
	                	bld.authno = Convert.newSubString(result[1], 46, 54);   //参考号8
	                	bld.trace = Integer.parseInt(Convert.newSubString(result[1], 54, 60));   //流水号6
//	            	   	bld.cardno = result[1].substring(18, 38);    //卡号20
//	                	bld.authno =result[1].substring(42, 50);   //参考号8
//	                	bld.trace = Integer.parseInt(result[1].substring(50, 56));   //流水号6
	            	}  	
	            	bld.retmsg = Convert.newSubString(result[1], 72, 172);    //错误信息
	            }
				return true;
	        }
			catch (Exception ex)
