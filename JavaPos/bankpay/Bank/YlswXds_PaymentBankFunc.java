...
public class YlswXds_PaymentBankFunc extends YlswWhf_PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			{
				PathFile.deletePath(ConfigClass.BankPath + "\\receipt.TXT");

				if (PathFile.fileExist(ConfigClass.BankPath + "\\receipt.TXT"))
				{
					errmsg = "签购单文件receipt.TXT无法删除,请重试";
					XYKSetError("XX", errmsg);
					new MessageBox(errmsg);
					return false;
				}
			}
			if (PathFile.fileExist(ConfigClass.BankPath + "\\transtotal.txt"))
			{
				PathFile.deletePath(ConfigClass.BankPath + "\\transtotal.txt");

				if (PathFile.fileExist(ConfigClass.BankPath + "\\transtotal.txt"))
				{
					errmsg = "结账文件transtotal.txt无法删除,请重试";
					XYKSetError("XX", errmsg);
					new MessageBox(errmsg);
					return false;
				}
	...
	public boolean XYKReadResult()
	{
		...
		try
		{
			...
			if (Convert.newSubString(result[1], 32, 44).trim().length() > 0)
			{
				...
				try
				{
					bld.je = ManipulatePrecision.doubleConvert(ManipulatePrecision.div(Double.parseDouble(Convert.newSubString(result[1], 32, 44)),
																						100), 2, 1);
				}
				catch (Exception er)
				{
	...
	public void XYKPrintDoc(int type)
	{
		...
		{
			printName = ConfigClass.BankPath + "\\transtotal.txt";
		}
