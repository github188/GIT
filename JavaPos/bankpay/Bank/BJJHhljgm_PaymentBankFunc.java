...
*红领巾工美建行银行接口
*/
public class BJJHhljgm_PaymentBankFunc extends PaymentBankFunc
{	
...
//			调用接口模块
			...
			{
				CommonMethod.waitForExec(getbankfunc()+"\\javaposbank.exe KeeperClient2", "javaposbank.exe");
			}
			else
			{
	...
	public String XYKgetRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		 ...
		 try
		 {
			...
			String fphm = Convert.increaseChar(String.valueOf(GlobalInfo.syjStatus.fphm), ' ', 6);
			// 根据要求拼接传入串
			line = type1+","+jestr+","+fphm+",,,,,"+strseqno+","+strauthno+","+TransDate+","+syjh+","+syyh;
			return line;
		 }
		 catch (Exception ex)
		 {
...
//			// 金额
			String je = result[17].trim();
			double j = Double.parseDouble(je);
			j = ManipulatePrecision.mul(j, 0.01);
			bld.je = j;
			// 卡号
			bld.cardno = result[6].trim();
			// 卡名称
			bld.bankinfo = result[20].trim();
			// 流水号
			bld.trace = Long.parseLong(result[10].trim());
			// 系统参考号
