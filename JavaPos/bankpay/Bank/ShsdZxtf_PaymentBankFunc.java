...
//中信泰富广场金卡接口
public class ShsdZxtf_PaymentBankFunc  extends PaymentBankFunc
{
	...
	Vector cardtype = null;
	 ...
	 public void XYKPrintDoc()
	 {
		 ...
		 try
		 {
			 ...
			 {
				 if (!PathFile.fileExist("c:\\JavaPOS\\bank\\SandTicket.txt"))
	             {
	             ...
	             {
	                  printName = "c:\\JavaPOS\\bank\\SandTicket.txt";
	             }
			 }
			 else
			 {
	...
	{
		if (cardtype != null) cardtype.clear();
		
		if (type != PaymentBank.XYKQD && type != PaymentBank.XYKJZ)
		{
			// 不进行选择，直接交易银联卡
			if (cardtype == null) cardtype = new Vector();
    ...
    protected String getBankClassConfig(String attr)
	{
		...
		if (attr.equals("REQCHECKDATETIME"))
		{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			if (type != PaymentBank.XYKQD && type != PaymentBank.XYKJZ && type != PaymentBank.XYKXF && type != PaymentBank.XYKCX && type != PaymentBank.XYKTH&& type != PaymentBank.XKQT1)
			{
				errmsg = "银联接口不支持该交易";
			...
			// 先删除上次交易数据文件
			if (PathFile.fileExist("c:\\javapos\\bank\\request.txt"))
			{
				PathFile.deletePath("c:\\javapos\\bank\\request.txt");
			...
			if (PathFile.fileExist("c:\\javapos\\bank\\result.txt"))
			{
				PathFile.deletePath("c:\\javapos\\bank\\result.txt");
            ...
            // 调用接口模块
            if (PathFile.fileExist("c:\\javapos\\bank\\javaposbank.exe"))
            {
            	CommonMethod.waitForExec("c:\\javapos\\bank\\javaposbank.exe SHSD");
            }
            else
            {
	...
	public boolean XYKWriteRequest(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector cardtype)
	{
		...
		try
		{
			...
			else if (type == PaymentBank.XKQT1) sbstr.append("90" + ",");
			else
			{
			...
			// 写入请求数据
			if (!rtf.writeFile("c:\\javapos\\bank\\request.txt", sbstr.toString()))
			{
	...
	public boolean XYKReadResult()
	{
		 ...
		 try
		 {
			 if (!PathFile.fileExist("c:\\javapos\\bank\\result.txt") || ((br = CommonMethod.readFileGBK("c:\\javapos\\bank\\result.txt")) == null))
			 {
             ...
             try
             {
                 ...
                 if (strs.length >= 13) bld.cardno = strs[12];
                 
                 if (strs.length >= 15) bld.bankinfo = strs[14];
                 if (bld.bankinfo.length() > 0 ) bld.bankinfo = XYKReadBankName(bld.bankinfo);
                 
                 if (strs.length >= 19) bld.authno = strs[18];
                 
                 if (strs.length >= 22 && strs[21] != null && strs[21].trim().length() > 0) bld.trace = strs[21].trim().length() > 0?Long.parseLong(strs[21].trim()):0;
             }
             catch (Exception ex)
             {
            	 ex.printStackTrace();
