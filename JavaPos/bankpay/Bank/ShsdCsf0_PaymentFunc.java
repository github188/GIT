...
public class ShsdCsf0_PaymentFunc extends PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			if (type != PaymentBank.XYKQD && type != PaymentBank.XYKCD && type != PaymentBank.XYKJZ && type != PaymentBank.XYKXF && type != PaymentBank.XYKCX && type != PaymentBank.XYKTH)
			{
				errmsg = "银联接口不支持该交易";
			...
			// 调用接口模块
			if (PathFile.fileExist("c:\\javapos\\bank\\SandCard.exe"))
			{
				CommonMethod.waitForExec("c:\\javapos\\bank\\SandCard.exe SHSD");
			}
			else
			{
				new MessageBox("找不到金卡工程模块 SandCard.exe");
				XYKSetError("XX", "找不到金卡工程模块 SandCard.exe");
				return false;
			}
