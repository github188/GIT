...
public class PbocCZK_PaymentBankFunc extends PaymentBankFunc
{	
	...
	public boolean XYKExecute(int type, double money, String track1,String track2, String track3, String oldseqno,String oldauthno, String olddate, Vector memo)
	{
		...
		try
		{
			...
			// 调用接口模块
			if (PathFile.fileExist(ConfigClass.BankPath + "\\bjbocom.exe"))
			{
...
//			 返回码
			...
			//bld.bankinfo = result[18];
			// 卡号
			bld.cardno = result[7];
			// 流水号
			//注意，当获取的字符全为空是，将导致转换异常
			if (result[13].trim().length()>0)
				bld.trace = Long.parseLong(result[13].trim());	
