...
public class ABACUS_PaymentBankFunc extends PaymentBankFunc {
	...
	public boolean XYKWriteRequest(int type, double money, String track1,
		...
		try {
			...
			line = "00"
					+ "\0"
					+ Convert.increaseChar("", ' ', 200)
					+ saletype
					+ "\0"
					+ Convert.increaseChar(GlobalInfo.posLogin.gh, ' ', 6)
					+ "\0"
					+ Convert
							.increaseChar(ConfigClass.CashRegisterCode, ' ', 6)
					+ "\0" + Convert.increaseCharForward(jestr, ' ', 12) + "\0"
	...
	public boolean XYKReadResult() {
		...
		try {
			...
			bld.je = ManipulatePrecision.doubleConvert(
					Double.parseDouble(kje) / 100, 2, 1);

			String kye = Convert.newSubString(line, 284, 297).trim();
			bld.kye = ManipulatePrecision.doubleConvert(
					Double.parseDouble(kye) / 100, 2, 1);

			bld.trace = Long.parseLong(Convert.newSubString(line, 297, 314)
					.trim());
