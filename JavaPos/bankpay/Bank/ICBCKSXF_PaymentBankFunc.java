...
//快速消费
public class ICBCKSXF_PaymentBankFunc extends PaymentBankFunc{

	String path = null;//宜春青龙项目更新
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			path = getBankPath(paycode);//宜春青龙项目更新
			// 调用接口模块
			if (PathFile.fileExist(path+"\\javaposbank.exe"))
			{
				...
				{
					PathFile.copyPath(path+"\\request.txt", path+"\\LastRequest.txt");
				...
				{
					PathFile.copyPath(path+"\\result.txt", path+"\\LastResult.txt");
...
typedef struct
{
	...
	char OperId[20];		//操作员号
				...
				CommonMethod.waitForExec(path+"\\javaposbank.exe ICBCKPCLIENT","javaposbank.exe");
			}
			else
			{
	...
	public void XYKPrintDoc()
	{
		...
		try
		{
			String printName = path+"\\ICBCPRTTKT.txt";
