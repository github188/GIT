...
public class YlswJHYB_PaymentBankFunc extends PaymentBankFunc
{
	...
	public boolean XYKExecute(int type, double money, String track1, String track2, String track3, String oldseqno, String oldauthno, String olddate, Vector memo)
	{
		try
		{
			...
			//设定路径
			searchBankPath("YlswJHYB");
			System.err.println("YlswJHYB " + bankpath);
			...
			{
				CommonMethod.waitForExec("C:\\JavaPOS\\javaposbank.exe YLSWSTBH", "javaposbank.exe");
			}
			else
			{
	...
	public boolean XYKReadResult1(int type)
	{
		...
		try
		{
			...
			if (type == PaymentBank.XYKJZ || type == PaymentBank.XYKCD || type == PaymentBank.XYKQD || type == PaymentBank.XYKCX)
			{
	...
	public String getErrorInfo(String retcode)
	{
		...
		try
		{
			if (!PathFile.fileExist(GlobalVar.ConfigPath + "\\bankError.txt") || !rtf.loadFile(GlobalVar.ConfigPath + "\\bankError.txt"))
			{
