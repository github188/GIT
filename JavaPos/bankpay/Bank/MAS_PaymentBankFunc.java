...
//马鞍山银联接口，调用动态库（模块名：MAS(和AQXB多一个传入分期字段)；动态库(dll文件）：KeeperClient.dll；函数：int __stdcall misposTrans(void* input, void* output)；）
public class MAS_PaymentBankFunc extends PaymentBankFunc
{
	String path = "C:\\gmc";
	String fq = "01";
		...
		try
		{
			...
			if(type == PaymentBank.XKQT2)
			{
	            ...
	            if(type == PaymentBank.XKQT1 || type == PaymentBank.XKQT2 || type == PaymentBank.XKQT3)
	            {
	            	...
	            	if(bf.toString().trim().matches("[1-9]{1}"))
	            	{
	            		fq = bf.toString();
	            	}
	            	else
	            	{
	            		new MessageBox("输入的期数不合法，请重新输入！");
	            		return false;
	            	}
	            }
			}
...
//		strncpy(inputData.FuncID,szRequest[10],4);
		...
		try
		{
	        ...
	        String FuncID = Convert.increaseChar("", ' ', 4); //分行特色脚本ID号4
			String trans = "";
			switch(type)
			{
...
//		strncpy(szResult[11],outputData.TerminalTraceNo,6);
		...
		try
		{
			...
			bld.retcode = str[6].trim();
			//当银行返回字符以字节计算，而字符中汉字出现导致line的长度和字节书不等时，汉字后面的内容倒着计算位置
			// int len = line.length();
			if (!bld.retcode.equals("00"))
			{
				bld.retmsg = bld.retcode + "," + str[10].trim();
