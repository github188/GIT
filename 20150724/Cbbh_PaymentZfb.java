...
import bankpay.o2o.tools.Md5Tools;
...
public class Cbbh_PaymentZfb extends Bcrm_PaymentMzk {
	...
	private Cbbh_PaymentZfbForm form = null;	

	private Vector<Cbbh_WCCRuleDef> wcc_rules = null;
	private Cbbh_WCCRuleDef wcc_rule = null;
				...
				if (!CommonMethod.isNull(bearXml.toString())) 
				{
					...
					if (xmldata != null) 
					{
						...
						{	
							...
							{
								//取消用券的商品
								markCommods(wcc_rule,Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE);								
							}
...
//		Md5Tools getMD5 = new Md5Tools();
								...
								if(parseRetRule(splits[1]))
								{
									if(findWCCRules(wcc_billno))
									{
										...
										if(rulesISMatch())
										{
											...
											{
												//把参与的商品打上已经使用标记
												markCommods(Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_USED + wcc_couponno);
											...
											{	
												markCommods(Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE);
												return false;
											}
										}
										else
	...
	protected boolean saveFindMzkResultToSalePay() {
		...
		salepay.batch = "";
		if(CommonMethod.isNull(salepay.payno)) salepay.payno = mzkret.cardno;
	...
	*/
	...
	{	
		...
		if(super.createSalePay(money))
		{
			...
			{
				//把参与的商品打上已经使用标记
				calcCommodJeFree(Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno);
				markCommods(wcc_rule,Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_USED + wcc_couponno);
	...
	{
		wcc_rules =  new Vector<Cbbh_WCCRuleDef>();
		if(!((Cbbh_DataService)DataService.getDefault()).findWCCRules(billno, wcc_rules)) return false;
		return true;
	}
	...
	public boolean rulesISMatch()
	{
		...
		for(i = 0; i < wcc_rules.size() ; ++ i)
		{
			...
			wcc_rule = (Cbbh_WCCRuleDef)wcc_rules.elementAt(i);
			//规则本身就是排除的,则不计算
			...
			for(j = 0 ; j < saleBS.saleGoods.size(); ++j)
			{
				...
				{	
					...
					{ 
						markCommod(wcc_rule,j,Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno);
						
						markCommods(wcc_rule,Cbbh_WCCRuleDef.WCC_CALC_MARK_SUM + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE);
					...
					//否则商品参与到金额统计
					zje += je;
					markCommod(wcc_rule,j,Cbbh_WCCRuleDef.WCC_CALC_MARK_SUM + wcc_couponno);
					
					//合计金额满足
					if(zje >= wcc_rule.value)
					{
						markCommods(wcc_rule,Cbbh_WCCRuleDef.WCC_CALC_MARK_SUM + wcc_couponno,Cbbh_WCCRuleDef.WCC_CALC_MARK_MATCH + wcc_couponno);
	...
	public boolean ruleMatchISNotAll(Cbbh_WCCRuleDef rule)
	{
		if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_ALL))
			return false;
		else return true;
	}
	...
	{
		sp.memo3 = Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE;
		sp.memo2 = Cbbh_WCCRuleDef.WCC_CALC_MARK_FREE;
	}
	...
	public boolean commodISUsed(String mode,int index)
	{
		...
		if(!mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_ALL) && sp.memo2.substring(0, 1).equals(Cbbh_WCCRuleDef.WCC_CALC_MARK_USED) ||
			mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_ALL) && sp.memo3.substring(0, 1).equals(Cbbh_WCCRuleDef.WCC_CALC_MARK_USED)
		  )
			return true;
	...
	public boolean ruleMatchCommod(Cbbh_WCCRuleDef rule,int index)
	{		
		//全场通用
		if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_ALL)) return true;
		
		SaleGoodsDef salegood = (SaleGoodsDef)(saleBS.saleGoods.elementAt(index));
		
		if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_GDID))
		{
			if (rule.gdid.equals(salegood.code)) return true;
			else return false;
		}
		else if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_CATID_AND_PPID))
		{
		...
		else if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_CATID))
		{
			if (rule.catid.equals(salegood.catid.substring(0, rule.catid.length()))) return true;
			else return false;
		}
		else if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_PPID))
		{
			if (rule.ppid.equals(salegood.ppcode)) return true;
			else return false;
		}
	...
	public boolean ruleISRemove(Cbbh_WCCRuleDef rule)
	{
		if(rule.isyq == 'N') return true;
		return false;
	}
	
	public boolean ruleNeedCalcRemove(Cbbh_WCCRuleDef rule)
	{
		if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_GDID)) return false;
		return true;
	}
	
	public void markCommod(Cbbh_WCCRuleDef rule,int index,String mark)
	{
		if(rule.mode.equals(Cbbh_WCCRuleDef.WCC_CALC_MODE_ALL))
	...
	public void markCommods(Cbbh_WCCRuleDef rule,String ma,String mb)
	{
		int i;
		SpareInfoDef sp;
		for(i = 0;i< this.saleBS.goodsSpare.size();++ i)
