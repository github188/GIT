...
/**
 * SaleRecord 表示商品销售流水记录. POS 系统采用以下方式完成销售流水记录: 销售过程中,客户端程序先把销售流水写入本地硬盘.
...
public class SaleRecord implements Serializable {
	...
	public SaleRecord(Element elm) {
		...
		batch = elm.getChild("batch").getTextTrim();
		yyyh = elm.getChild("yyyh").getTextTrim();
		cfly = elm.getChild("cfly").getTextTrim();
		cfshr = elm.getChild("cfshr").getTextTrim();
		cfdpr = elm.getChild("cfdpr").getTextTrim();
		cffhr = elm.getChild("cffhr").getTextTrim();
		gmsfz = elm.getChild("gmsfz").getTextTrim();
	}
	...
	public String toString() {
		return vgno + " * " + qty + " at " + systime.toString() + " on "
				+ sysdate.toString();
	}

	// ///////////////// internal routines ////////////////////
	private static Day str2day(String sysdate) {
		return new Day(atoi(sysdate.substring(0, 4)), atoi(sysdate.substring(5,
				7)), atoi(sysdate.substring(8, 10)));
	}
