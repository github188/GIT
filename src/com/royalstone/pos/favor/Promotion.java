...
public class Promotion extends DiscCriteria {
	 ...
	 *            截止日期与时间
	 */
	public Promotion(String vgno, int price, GregorianCalendar start,
	...
	public Promotion(String vgno, int price, GregorianCalendar start,
			GregorianCalendar end, String flag, int qty) {
		super(start, end);
		this.vgno = vgno;
		this.price = price;
		this.flag = flag;
	...
	/**
	 * 根据传入的参数决定是否可以享受促销折扣. 此函数实现父类中的抽象方法. cust_level(客户级别)被忽略.
	 * 
	...
	public boolean matches(Goods goods, int qty, int cust_level) {
		...
		if (this.flag.equals("1")) 
			return (super.valid()
					&& (goods.getPType() != null && goods.getPType().equals(
							DiscCriteria.PROMOTION)) && vgno.equals(goods
					.getVgno()) && this.qty >= qty);
		else
			return (super.valid()
					&& (goods.getPType() != null && goods.getPType().equals(
							DiscCriteria.PROMOTION)) && vgno.equals(goods
	...
	/**
	 * 根据传入的参数决定商品可以享受的折扣. 此函数实现父类中的抽象方法. cust_level(客户级别)被忽略.
	 * 
	...
	public Discount getDiscount(Goods goods, int qty, int cust_level) {
		if (matches(goods, qty, cust_level))
			return new DiscPrice(Discount.PROMOTION, price);
